...
`data/` contains library metadata, settings, credentials or sessions, playback state, sync state, deletion reconciliation state, optional push subscriptions, and custom-voice metadata. `cache/` contains imported material or extracted playback artifacts, covers, voice samples, generated narration, and compact offline derivatives. A full offline download has two explicit milestones. **Prepare for offline** runs durably on the server, reusing completed narration and transcoding a separate mono 24 kHz, 48 kbps MP3 package without rerunning TTS. It survives a browser close, processes one chapter at a time for each title across at most two titles, and can send an opt-in Web Push notification when the whole title is ready. **Download to this device** appears only after every compact chapter exists; it transfers bytes into the signed-in account's browser/PWA cache and reports actual bytes, measured throughput, and ETA. The transfer requires an online connection, Cache Storage, and a foreground PWA on platforms that suspend background networking. Xandrio requests a screen wake lock and eviction-resistant storage where supported. Every verified chapter is playable as soon as it reaches the device; interrupted transfers retain completed chapters and resume when Xandrio is reopened online. Canceling a device transfer leaves the prepared server package intact; canceling preparation releases only that device's preparation request. Deleting a title cancels preparation, removes server artifacts, and publishes a durable deletion record so devices remove older local copies when they reconnect.
