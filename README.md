...
`data/` contains library metadata, settings, credentials or sessions, playback state, sync state, deletion reconciliation state, and custom-voice metadata. `cache/` contains imported material or extracted playback artifacts, covers, voice samples, and generated audio. A full offline download stores verified chapter audio plus a compact title/chapter snapshot in that browser/PWA, scoped to the signed-in account. The Downloaded view therefore describes only the current device and browser profile. **Download to this device** starts durable, shared server preparation when chapters are missing; the server materializes one chapter at a time for each title, across at most two titles, while live playback keeps priority. Preparation survives a browser close, and each requesting device transfers ready chapters into its own local cache. Every verified chapter becomes playable offline as soon as it reaches the device, without waiting for the full title. Canceling on one device releases only that device's request; shared preparation pauses when no devices still request it. Device download requires an online connection and browser Cache Storage support. Xandrio requires a foreground-transfer acknowledgement and requests a screen wake lock where supported, but switching apps, locking the screen, or closing the PWA can still suspend the transfer. An interrupted download retains its playable chapters and automatically resumes when the PWA is reopened online; its active chapter may restart. Xandrio also asks the browser for eviction-resistant storage when a download starts, but browsers may deny it. Deleting a title cancels its preparation, removes its server artifacts, and publishes a durable deletion record; each device removes an older local copy when it next reconnects.
