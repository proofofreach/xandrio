...
(async()=>{
 ...
 const doc=createBookDocument({extractEpubChapters:async()=>{throw Error('v30 migration must preserve cached text and boundaries');},log:{log(){},warn(){},error(){}}});
 ...
 const output=path.join(root,'cache-migration-preserved-e2e.json');await fs.writeFile(output,JSON.stringify({pass:true,source,cache,textUnchanged:true,createdAt:new Date().toISOString()},null,2),{flag:'wx'});console.log(output);
