...
| Sequence/title repair | A complete generic numeric or Roman-numeral series with local structural evidence, or a merged divider heading adjacent to a host still labelled with a generic ordinal that is not part of a contiguous numbered run — a divider standing before every excerpt names each one, while a divider before a run of adjacent numbered chapters is a heading over all of them and they keep their numbers | Changes display metadata, not narration text; `rawTitle` retains the original, so the chapter structure key is unaffected | `test-chapter-utils.js`, `test-epub-parser.js` |
