...
| IOS-01 | Blocking | Safari on iOS | Physical supported iPhone/iPad Safari plays a generated chapter, seeks through a Range request, locks/unlocks, resumes, uses the sleep timer, and installs/opens the PWA if offered. Record device, iOS, Safari version, network, and result. | Pass | Owner ran the full `IPHONE-TEST.md` checklist on a physical iPhone against the current `main` on 2026-08-29; owner to append device, iOS, and Safari versions. Re-run against the immutable candidate before promotion. | QA owner | 2026-08-29 |
