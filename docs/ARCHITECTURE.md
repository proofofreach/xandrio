...
| Book Guide services (experimental) | PPQ.ai guide generation, deterministic evidence validation, semantic verification, versioned guide artifacts, durable jobs, and section-level TTS through the shared narration queue; disabled by default |
...
Book Guides are an instance-level experimental feature and default to off. The admin acknowledges external processing once when configuring PPQ.ai, then explicitly tags eligible English nonfiction titles. Xandrio sends bounded source segments and verification evidence through PPQ.ai with zero-data-retention routing required. A durable guide job snapshots the source fingerprint and chapter structure, extracts candidate claims, composes a concept-first guide, validates evidence anchors and quote controls, semantically verifies every material claim and recall answer, and publishes atomically. Provider credentials are write-only and stored separately from guide configuration. The guide stores version/model-route/recipe provenance and evidence ranges, not a replacement source copy. A deterministic narration projection turns that artifact into short sections and sends them through the existing active-voice TTS queue on demand. Its cache identity includes the guide version and voice variant. Source or chapter-structure changes mark the guide stale and revalidate its anchors. Replacement and deletion prune obsolete guide audio. [BOOK_GUIDES.md](BOOK_GUIDES.md) defines certification, privacy, cost, and evaluation gates; fiction is outside V1.
...
Supported operator modes are localhost/native Node.js, Docker or Docker Compose, and a private remote path such as Tailscale or a TLS reverse proxy. Local engines require a separately managed local model/runtime in the standard container path. Read [SELF_HOSTING.md](SELF_HOSTING.md) before exposing an instance to a network, and [DEPLOYMENT_TOPOLOGY.md](DEPLOYMENT_TOPOLOGY.md) for the reference split between a public web instance (accounts, Edge TTS) and a local engine host (Kokoro/Chatterbox).
