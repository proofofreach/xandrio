...
(async()=>{
 ...
 let admit,admittedCalls=0;const admitted=new Promise(r=>admit=r);
 const cancellable=createJevGuideVerifier({enabled:true,acknowledged:true,apiKey:'test',evaluate:async()=>{if(++admittedCalls===1){admit();return new Promise(()=>{});}return good();}});
 const admittedAbort=new AbortController();const cancelledCall=cancellable.verify(items,{fallback,signal:admittedAbort.signal});const cancelledResult=assert.rejects(cancelledCall);await admitted;admittedAbort.abort();await cancelledResult;await cancellable.verify(items,{fallback});assert.equal(admittedCalls,2,'user cancellation must not open outage circuit');
