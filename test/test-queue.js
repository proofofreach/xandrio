...
async function runTests() {
  ...
  await test('a job whose output is already rendered skips synthesis', async () => {
    const os = require('os');
    const fsp = require('fs/promises');
    const nodePath = require('path');
    const dir = await fsp.mkdtemp(nodePath.join(os.tmpdir(), 'xandrio-reuse-'));
    try {
      const q = new TestableQueue({ maxConcurrent: 1, generateDelay: 5 });
      const rendered = nodePath.join(dir, 'already-there.mp3');
      await fsp.writeFile(rendered, 'pretend audio');

      const reusedId = await q.enqueue({
        text: 'should not be spoken',
        outputPath: rendered,
        priority: 'immediate',
        reuseExistingOutput: true
      });
      assert.strictEqual(await q.waitFor(reusedId), rendered, 'it still resolves normally');
      assert.strictEqual(q.generationLog.length, 0, 'TTS was never invoked');

      // An empty file is a truncated write, not reusable audio.
      const truncated = nodePath.join(dir, 'truncated.mp3');
      await fsp.writeFile(truncated, '');
      const regeneratedId = await q.enqueue({
        text: 'must be spoken',
        outputPath: truncated,
        priority: 'immediate',
        reuseExistingOutput: true
      });
      await q.waitFor(regeneratedId);
      assert.strictEqual(q.generationLog.length, 1, 'a zero-length artifact is regenerated');

      // Without the flag, the caller means "render this again".
      const forcedId = await q.enqueue({
        text: 'explicit retry',
        outputPath: rendered,
        priority: 'immediate'
      });
      await q.waitFor(forcedId);
      assert.strictEqual(q.generationLog.length, 2, 'reuse stays opt-in');
    } finally {
