...
function parsePdfInfo(stdout) {
  ...
  for (const line of String(stdout || '').split(/\r?\n/)) {
    ...
    if (key === 'language') info.language = value;
  }
...
function matchesPlanEntry(page, entry) {
  const top = normalizeStructureText(String(page.text || '').slice(0, 1800));
  if (!top) return false;
...
function resolveTocPlan(plan, pages = []) {
  ...
  const resolved = plan.entries.map(entry => {
    const direct = bodyPages.find(page => matchesPlanEntry(page, entry));
...
function buildTocChapters(pages, options = {}) {
  ...
  for (let index = 0; index < resolved.entries.length; index += 1) {
    ...
    const next = resolved.entries.slice(index + 1).find(candidate => candidate.pdfPage > entry.pdfPage);
