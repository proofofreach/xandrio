...
async function readPdfOutline(pdfPath, fs = require('fs').promises) {
  ...
  try {
    ...
    loadingTask = pdfjs.getDocument({ data, disableWorker: true, isEvalSupported: false });
    ...
    // pdfjs-dist 6 removed PDFDocumentProxy.destroy(); the loading task owns teardown.
...
async function extractPdfResult(pdfPath, options = {}) {
  ...
  // Optional outer judgment pass: keep synchronous normalization deterministic.
  // Only the already-selected extractor is repaired; selection itself is unchanged.
  if (options.repairPages && result.selected.rawPages?.length) {
    try {
      const repair = await options.repairPages(result.selected.rawPages);
      if (repair?.report) {
        result.sourceDocument.jevRepairs = repair.report;
        if (repair.report.accepted.length) {
          const normalized = normalizePdfPages(repair.pages);
          const built = buildPdfChaptersFromCandidate(sourceLabel, normalized.pages,
            normalized.pages.map(page => page.text).join('\n\n'), normalized.diagnostics,
            result.selected.name, result.sourceDocument.outline, repair.pages);
          Object.assign(result.selected, built, { sourcePages: normalized.pages, normalization: normalized.diagnostics });
          result.sourceDocument.pages = normalized.pages.map(page => ({ pageNumber: page.pageNumber, text: page.text }));
          result.sourceDocument.rawPages = repair.rawPages.map(({ pageNumber, text }) => ({ pageNumber, text }));
          result.sourceDocument.structure = built.structure;
          if (result.selected.chapters[0]) result.selected.chapters[0].pdfExtraction = result.extractionReport;
        }
      }
    } catch {
      // Optional enrichment must never prevent an otherwise valid local import.
    }
  }
