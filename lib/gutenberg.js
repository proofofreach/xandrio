...
const { requestRemote, declaredLength, readBoundedBuffer } = require('./remote-fetch');
const { streamResponseToFile } = require('./remote-download');
const { assertAllowedDownloadHost, GUTENBERG_HOSTS } = require('./download-hosts');
...
function remoteOptions(timeoutMs) {
  return {
    ...
    // Gutendex's CDN bot-walls this server's direct egress IP (403), so
    // search must exit through the egress proxy when one is configured.
    proxyUrl: process.env.BOOK_PROXY_URL || undefined
  };
}

function isTransientSearchError(err) {
  const message = String(err?.message || '');
  if (/exceeds the allowed size|not a safe public|redirect/.test(message)) return false;
  return Number(err?.statusCode) >= 500 || /aborted|timed out|timeout|socket|fetch failed|50[0234]/i.test(message);
}
...
async function fetchJsonOnce(url, timeoutMs) {
  ...
  try {
    if (!remote.response.ok) {
      throw Object.assign(new Error(`Gutendex API error: ${remote.response.status} ${remote.response.statusText}`), {
        statusCode: remote.response.status
      });
    }
...
async function fetchJson(url, timeoutMs = SEARCH_TIMEOUT_MS) {
  try {
    ...
    // Gutendex's edge intermittently stalls or answers a bare 5xx; one quick
    // retry rides out short waves before the search falls back to the cache.
    if (!isTransientSearchError(err)) throw err;
...
async function downloadBook(gutenbergId, downloadUrl, destPath) {
  // The URL arrives from the client alongside `source: 'gutenberg'`, which is
  // what exempts the import from the unverified-source acknowledgement. Bind
  // the two together so the exemption cannot be borrowed for another host.
  assertAllowedDownloadHost(downloadUrl, GUTENBERG_HOSTS, 'Project Gutenberg');
  ...
  try {
    ...
    await streamResponseToFile(response, destPath, MAX_DOWNLOAD_BYTES);
  } finally {
    remote.close();
  }
