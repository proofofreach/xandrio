...
function createOfflineTransferManifest({ getBookChapters, chapterStatus, preparationStatus, preparationIdentity }) {
  return async function manifest(bookId) {
    ...
    const status = await preparationStatus(bookId, identity);
    ...
    const sourceRevision = identity.retained ? digest([
      'retained-ready', identity.sourceTextRevision, identity.packageVariantKey, identity.retainedArtifacts
    ]) : digest([
      textRevision,
      identity.sourceTextRevision || '',
