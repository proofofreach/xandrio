...
async function load(filePath, defaultValue = {}) {
  ...
  try {
    ...
    // A missing file is the ordinary first-run path. Returning defaults for
    // any other failure lets update() replace an unreadable store wholesale.
...
async function writeAtomic(filePath, data) {
  ...
  try {
    ...
    // Covers partial writes and rename failures. After a successful rename
    // the temp path no longer exists, so this is a harmless ENOENT.
