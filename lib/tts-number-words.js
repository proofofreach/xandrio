...
function scaledAmountToWords(amount, scale, symbol) {
  const match = String(amount).match(/^(\d+|\d{1,3}(?:,\d{3})+)(?:\.(\d+))?$/);
  if (!match || !CARDINAL_SCALES.slice(1).includes(scale)) return null;
  const whole = integerToCardinalWords(match[1]);
  if (!whole) return null;
  const fraction = match[2] ? ` point ${[...match[2]].map(digit => CARDINAL_SMALL[Number(digit)]).join(' ')}` : '';
  const currency = symbol ? CURRENCY_NAMES[symbol] : null;
  if (symbol && !currency) return null;
  return `${whole}${fraction} ${scale}${currency ? ` ${currency.majorPlural}` : ''}`;
}
