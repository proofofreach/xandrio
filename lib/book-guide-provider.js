...
const PPQ_GUIDE_MODELS = new Set([DEEPSEEK_FLASH_MODEL, DEEPSEEK_PRO_MODEL, GLM_VERIFIER_MODEL]);
...
function createPpqBookGuideProvider({
  ...
  async function generate({ baseUrl = PPQ_BASE_URL, modelSnapshot, prompt, purpose = 'generation', signal = null }) {
    ...
    if (!response.ok) {
      ...
      throw providerError(`PPQ.ai request failed (${response.status})`, code, response.status === 402 ? 409 : 503);
    }
    const data = await boundedJson(response, maxResponseBytes);
    if (data.model && data.model !== expected.name) {
      throw providerError('PPQ.ai substituted a different model', 'BOOK_GUIDE_MODEL_SUBSTITUTED', 409);
    }
