...
async function createPinnedBrowserProxy(options = {}) {
  ...
  server.on('connect', (request, clientSocket, head) => {
    // A tunnelled client that resets mid-transfer (the common case when the
    // browser navigates away) emits 'error' on this socket. Without a
    // listener that is an unhandled 'error' event, which takes the whole
    // server down — pipe() below would not forward it either.
    ...
    void (async () => {
      ...
      // Tear the tunnel down symmetrically so a dead client cannot strand the
      // upstream connection.
