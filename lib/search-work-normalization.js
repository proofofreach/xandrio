...
const EDITION_DESCRIPTOR_PATTERN = String.raw`(?:the\s+|a\s+)?(?:` +
  String.raw`original\s+scroll|original\s+manuscript|author['’]?s\s+preferred|` +
  String.raw`new\s+snapshots|reader['’]?s|` +
...
function profilesForCreatorName(name) {
  const profiles = [];
...
// Work identity leads with the primary creator. Providers disagree about who
// that is, so contributorIdentityProfiles() exposes the rest of the credits for
// the bounded title-anchored repair in search-work-resolution.
function creatorIdentityProfiles(author) {
  return [...new Set(creatorNames(author).slice(0, 1).flatMap(profilesForCreatorName))];
}

// One profile set per credited contributor, in listed order. Comparing whole
// sets keeps a contributor match as strict as a primary-creator match: the same
// initials-versus-full-name rules apply, so "C G Jung" and "Jung, Carl Gustav"
// are recognised as one person while unrelated contributors are not.
function contributorIdentityProfiles(author) {
  return creatorNames(author)
    .map(name => [...new Set(profilesForCreatorName(name))])
...
// Providers place the same edition label in different slots: "The Red Book: A
// Reader's Edition", "…: A Reader's Edition: Liber novus", "…: A Reader's
// Edition: A Reader's Edition". Stripping the label only when it trails leaves
// those as different works. Drop any colon-delimited segment that is nothing
// but an edition label, never the leading one, so a real title that happens to
// open with such a word ("Kindle: A History") keeps its subject.
const EDITION_LABEL_SEGMENT = new RegExp(
  String.raw`^\s*${EDITION_ORDINAL_PATTERN}(?:${EDITION_LABEL_PATTERN})(?:\s+(?:edition|version))?\s*$`,
  'i'
);

function stripEditionLabelSegments(title) {
  const value = boundedScanText(title);
  if (!value.includes(':')) return value;
  const segments = value.split(':');
  const kept = segments.filter((segment, index) => index === 0 || !EDITION_LABEL_SEGMENT.test(segment));
  return kept.length === segments.length ? value : kept.join(':');
}

// Calibre-style catalog prefixes such as "[Liar's Poker 01] • Liar's Poker"
// name a series or shelf label. Identity and volume/derivative qualifiers
// use the remainder after the leading "[…]" and an optional bullet or dash.
function catalogPrefixRemainder(title) {
  const value = boundedScanText(title);
...
function canonicalWorkTitle(title) {
  let cleaned = stripEditionLabelSegments(title)
