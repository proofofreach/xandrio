...
// Rewrite the request authority to the literal address assertPublicTarget
// approved, preserving any explicit port, so a CONNECT tunnel (or a proxied
// fetch) cannot dial a different, unvetted address after validation — the
// same DNS-rebinding gap `pinnedHttpsFetch` closes for the direct path.
function pinnedProxyTarget(url, target) {
  const literalHost = target.family === 6 ? `[${target.address}]` : target.address;
  const rewritten = new URL(url.href);
  rewritten.host = url.port ? `${literalHost}:${url.port}` : literalHost;
  return rewritten;
}

// Keep the original Host header (unless the caller set one explicitly) so
// certificate validation and virtual hosting are unaffected by the pinned
// authority rewrite above.
function withOriginalHost(url, headers) {
  const merged = { ...headers };
  if (!Object.keys(merged).some(name => name.toLowerCase() === 'host')) merged.host = url.host;
  return merged;
}
...
function proxiedHttpsFetch(url, options, target, proxyUrl, requestImpl = https.request) {
  return new Promise((resolve, reject) => {
    const headers = new Headers();
    const request = requestImpl(pinnedProxyTarget(url, target), {
      method: options.method || 'GET',
      headers: withOriginalHost(url, options.headers),
...
function pinnedUndiciFetch(url, options, target, {
  ...
  let dispatcher;
  if (proxyUrl) {
    requestUrl = pinnedProxyTarget(url, target);
    requestHeaders = withOriginalHost(url, options.headers);
...
function transportFor(options, dispatchers) {
  ...
  if (options.requestImpl && !options.directFetchImpl) {
    return (url, requestOptions, target) => options.proxyUrl
      ? proxiedHttpsFetch(url, requestOptions, target, options.proxyUrl, options.requestImpl)
