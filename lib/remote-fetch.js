...
// The CONNECT authority is the address assertPublicTarget already vetted for
// this hop, not the hostname, so the proxy cannot be tricked into resolving
// (and dialing) a different, unvetted address after validation — the same
// DNS-rebinding gap `pinnedHttpsFetch` closes for the direct path. SNI and
// the Host header stay the original hostname so certificate validation and
// virtual hosting are unaffected.
function proxiedHttpsFetch(url, options, target, proxyUrl, requestImpl = https.request) {
  return new Promise((resolve, reject) => {
    ...
    const connectUrl = new URL(url.href);
    connectUrl.host = literalHost;
    const requestHeaders = { ...options.headers };
    if (!Object.keys(requestHeaders).some(name => name.toLowerCase() === 'host')) {
      requestHeaders.host = url.host;
    }
    const request = requestImpl(connectUrl, {
...
function pinnedUndiciFetch(url, options, target, {
  ...
  if (proxyUrl) {
    ...
    dispatcher = proxyDispatcherFactory({
      ...
      connections: 1,
      maxOrigins: 1,
      requestTls: { servername: url.hostname.replace(/^\[|\]$/g, '') }
    });
  } else {
  ...
  try {
    for (let redirects = 0; redirects <= maxRedirects; redirects += 1) {
      ...
      const response = options.fetchImpl
        ...
        : options.requestImpl && !options.directFetchImpl
          ? options.proxyUrl
            ? await proxiedHttpsFetch(current, requestOptions, target, options.proxyUrl, options.requestImpl)
            : await pinnedHttpsFetch(current, requestOptions, target, options.requestImpl)
          : await pinnedUndiciFetch(current, requestOptions, target, {
