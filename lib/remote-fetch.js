...
async function requestRemote(initialUrl, options = {}) {
  ...
  try {
    for (let redirects = 0; redirects <= maxRedirects; redirects += 1) {
      ...
      // Opt-in: hand the redirect back instead of throwing, for callers that must
      // read its headers (an edge that gates on a Set-Cookie handshake) and decide
      // for themselves whether to replay. Nothing is followed on this path, so the
      // caller re-enters through requestRemote and is re-validated from scratch.
      if (redirects === maxRedirects) {
        if (options.redirectAsResponse) return { response, close };
