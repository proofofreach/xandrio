...
const STATIC_NOISE_POLICY = Object.freeze({
  minimumFrames: 20,
  maximumMeanSpectralFlatness: 0.45,
  flatFrameThreshold: 0.45,
  minimumStaticRunFrames: 20
});
...
function assessStaticNoiseProfile(flatnessValues, policy = STATIC_NOISE_POLICY) {
  const values = Array.isArray(flatnessValues)
    ? flatnessValues.map(Number).filter(Number.isFinite)
    : [];
  const meanSpectralFlatness = values.length
    ? values.reduce((sum, value) => sum + value, 0) / values.length
    : null;
  let consecutiveFlatFrames = 0;
  let maximumConsecutiveFlatFrames = 0;
  for (const value of values) {
    consecutiveFlatFrames = value >= policy.flatFrameThreshold ? consecutiveFlatFrames + 1 : 0;
    maximumConsecutiveFlatFrames = Math.max(maximumConsecutiveFlatFrames, consecutiveFlatFrames);
  }
  const staticNoise = values.length >= policy.minimumFrames
    && (
      meanSpectralFlatness >= policy.maximumMeanSpectralFlatness
      || maximumConsecutiveFlatFrames >= policy.minimumStaticRunFrames
    );
  return {
    staticNoise,
    frameCount: values.length,
    meanSpectralFlatness,
    maximumConsecutiveFlatFrames
  };
}

function measureStaticNoiseProfile(filePath, { analysisSeconds = 30 } = {}) {
  return new Promise((resolve) => {
    execFile('ffmpeg', [
      '-hide_banner', '-nostats', '-loglevel', 'info',
      '-t', String(analysisSeconds),
      '-i', filePath,
      '-af', 'aspectralstats=win_size=2048:overlap=0.5,ametadata=print:key=lavfi.aspectralstats.1.flatness',
      '-f', 'null', '-'
    ], { maxBuffer: 4 * 1024 * 1024 }, (error, _stdout, stderr) => {
      if (error) return resolve(null);
      const values = [...String(stderr).matchAll(/lavfi\.aspectralstats\.1\.flatness=([0-9.eE+-]+)/g)]
        .map(match => Number(match[1]))
        .filter(Number.isFinite);
      resolve(assessStaticNoiseProfile(values));
    });
  });
}
...
module.exports = {
  ...
  assessStaticNoiseProfile,
  measureStaticNoiseProfile,
