...
function normalizeChapterMetadata(chapter = {}, work = null) {
  ...
  // Gutenberg's standalone legal appendix may have a trusted EPUB heading.
  // Identify the actual license before that heading's early return, without
  // treating prose that merely mentions Gutenberg as publication furniture.
  if (
    /^The Full Project Gutenberg[™]? License$/i.test(String(chapter.title || '').trim()) &&
    /^\s*\*\*\* END OF THE PROJECT GUTENBERG EBOOK\b/.test(String(chapter.text || '')) &&
    /Section 1\.\s+General Terms of Use and Redistributing Project Gutenberg electronic works/i.test(String(chapter.text || '').slice(0,5000))
  ) {
    return { ...chapter, type: 'copyright' };
  }
  if (
  ...
  if (chapter.tocHierarchy === true && ['frontmatter', 'backmatter'].includes(chapter.type)) {
    return chapter;
  }
  ...
  const trustedDocumentHeading = chapter.titleSource === 'heading' || chapter.titleSource === 'spine' ||
    chapter.titleSource === 'printed-page-heading';
