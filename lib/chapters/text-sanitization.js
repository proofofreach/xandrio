...
function stripHTML(html, options = {}) {
  ...
  // PRE whitespace is authored layout, unlike pretty-printed HTML source wraps.
  // Preserve newlines as explicit breaks before general whitespace/dehyphenation.
  text = text.replace(/<pre\b([^>]*)>([\s\S]*?)<\/pre\s*>/gi, (_match, attrs, body) =>
    `<pre${attrs}>${body.replace(/(<[^>]*>)|\r\n?|\n/g, (match, tag) => tag || '<br>')}</pre>`);
  ...
  text = text
    ...
    .replace(/<\/(p|div|pre|h[1-6]|li|br\s*\/?)>/gi, '\n')
    .replace(/<br\s*\/?>/gi, '\n')
    .replace(/<[^>]*>/g, ' ')
    .replace(/&nbsp;/gi, ' ')
    .replace(/&#160;/gi, ' ')
