...
const { assertAllowedDownloadHost, INTERNET_ARCHIVE_HOSTS } = require('../download-hosts');
...
async function download(result, destPath) {
  ...
  // `downloadUrl` is client-supplied; `fileUrl()` is server-built. Only the
  // former can point elsewhere, and both go through the same check.
  assertAllowedDownloadHost(url, INTERNET_ARCHIVE_HOSTS, 'Internet Archive');
