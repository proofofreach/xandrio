...
const { isPublicAddress, isSafeRemoteUrl, requestRemote, declaredLength, readBoundedBuffer } = require('../remote-fetch');
const { streamResponseToFile } = require('../remote-download');
const { assertAllowedDownloadHost, opdsAllowedHosts } = require('../download-hosts');
...
function createOpdsProvider(options = {}) {
  ...
  async function fetchFeed() {
    ...
    try {
      ...
      if (response.status === 401) {
        const hint = username ? '' : ' (set the OPDS account credentials to enable this source)';
        throw Object.assign(new Error(`${label} OPDS feed requires authentication${hint}`), {
          statusCode: 401
        });
      }
  ...
  async function download(result, destPath) {
    ...
    // The URL arrives from the client, which means naming this provider was
    // enough to make the server fetch any host at all -- and to file the result
    // under this provider's rights provenance. An OPDS catalog may legitimately
    // serve its files from a different origin than the feed, so the host cannot
    // simply be pinned to the feed's; instead the URL must be one the feed
    // itself currently publishes. That keeps CDN-hosted catalogs working while
    // removing the client's choice of destination entirely.
    const published = await fetchFeed();
    if (!published.some(entry => entry.downloadUrl === result.downloadUrl)) {
      assertAllowedDownloadHost(result.downloadUrl, opdsAllowedHosts(feedUrl), label);
    }
    ...
    try {
      ...
      await streamResponseToFile(response, destPath, maxDownloadBytes);
    } finally {
      remote.close();
    }
  }
