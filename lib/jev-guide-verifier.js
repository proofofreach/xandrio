...
function createJevGuideVerifier({ enabled = false, acknowledged = false,
  ...
  async function verify(items, { fallback, signal } = {}) {
    ...
    if (!active || busy || Date.now() < unavailableUntil || !items.length || items.length > 24) return fallback(items);
    ...
    if (Buffer.byteLength(JSON.stringify(state)) > 180000) return fallback(items);
    ...
    try {
      ...
      if (route?.canonicalSlug !== 'typesafe-ai/jev' || route.finalProvider !== 'typesafe-ai' ||
    ...
    signal?.throwIfAborted();
    if (!probabilities) return fallback(items);
    ...
    return { verdicts: items.map((item, i) => ({ claimId: item.id,
