...
function createUserLibraryState(options = {}) {
  ...
  function setBookPositionsStructureKey(rawPositions, bookId, chapterStructureKey, previousStructureKey) {
    const positions = normalizePositionsStore(rawPositions);
    Object.values(positions.users || {}).forEach(userPositions => {
      const position = userPositions?.[bookId];
      if (position && typeof position === 'object' &&
          (previousStructureKey === undefined || position.chapterStructureKey === previousStructureKey)) {
        position.chapterStructureKey = chapterStructureKey;
      }
    });
    return positions;
  }
