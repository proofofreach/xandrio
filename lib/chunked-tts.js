...
const CONCAT_TIMEOUT_MS = 30 * 60 * 1000;
const PROCESS_TERMINATION_GRACE_MS = 2000;
...
function runAbortableCommand(command, args, {
  signal,
  ...
  return new Promise((resolve, reject) => {
    ...
    const finish = (error, stdout, stderr) => {
      ...
      if (aborted) return reject(Object.assign(new Error('Generation intent was retired'), {
        name: 'AbortError'
      }));
      if (timedOut) {
        const timeoutError = new Error('ffmpeg concat timed out');
        timeoutError.code = 'TTS_CONCAT_TIMEOUT';
    ...
    const terminate = () => {
      if (!child || settled || child.exitCode !== null || child.signalCode) return;
      child.kill('SIGTERM');
    ...
    child = exec(command, args, finish);
    signal?.addEventListener('abort', abort, { once: true });
    if (signal?.aborted) abort();
    if (!settled) {
      const boundedTimeout = Math.max(1, Math.min(CONCAT_TIMEOUT_MS, Number(timeoutMs) || CONCAT_TIMEOUT_MS));
      timeout = setTimeout(() => {
        timedOut = true;
        terminate();
      }, boundedTimeout);
...
class ChunkedTTS extends EventEmitter {
  ...
  async _buildManifest(bookId, chapterIndex, chunkMetaOrTexts, options = {}) {
    ...
    const chunks = await Promise.all(chunkTexts.map(async (chunkText, i) => {
      ...
      if (this.queue?.reuseRenderedOutput) {
        exists = Boolean(await this.queue.reuseRenderedOutput(this._renderedOutputRequest(
          bookId,
          chapterIndex,
          chunkMeta[i],
          i,
          chunkTexts.length,
          { language: options.language || 'en', voice, variantKey }
        )));
      } else {
        exists = await this._fileExists(p);
      }
  ...
  async reconstructChapterManifest(bookId, chapterIndex, text, language = 'en') {
    ...
    if (chunkMeta.length === 0) throw Object.assign(new Error('Chapter has no speakable text for TTS'), { code: 'CHAPTER_UNSPEAKABLE' });
  ...
  async chapterArtifactReusePlan(bookId, chapterIndex, text, language = 'en', options = {}) {
    ...
    const artifacts = chunkMeta.map((chunk, index) => {
      const request = this._renderedOutputRequest(
        bookId,
        chapterIndex,
        chunk,
        index,
        chunkMeta.length,
        { language, voice, variantKey }
      );
      return {
        outputPath: request.outputPath,
        fingerprint: this.queue.renderedOutputFingerprint(request)
      };
  ...
  async indexChapterArtifacts(bookId, chapterIndex, text, language = 'en', options = {}) {
    ...
    for (let index = 0; index < chunkMeta.length; index++) {
      if (await this.queue.indexRenderedOutput(this._renderedOutputRequest(
        bookId,
        chapterIndex,
        chunkMeta[index],
        index,
  ...
  async generateChapter(bookId, chapterIndex, text, language = 'en', priority = 'immediate', options = {}) {
    ...
    if (chunkTexts.length === 0) {
      throw Object.assign(new Error('Chapter has no speakable text for TTS'), { code: 'CHAPTER_UNSPEAKABLE' });
    }
    const variantKey = String(this.variantKeyProvider() || 'default');
  ...
  _renderedOutputRequest(bookId, chapterIndex, chunk, chunkIndex, totalChunks, {
    language,
    voice,
    variantKey
  }) {
    const render = this._chunkRenderOptions(chunk, chunkIndex, totalChunks);
    return {
      text: chunk.text,
      outputPath: this.chunkPath(bookId, chapterIndex, chunkIndex),
      language,
      voice,
      padEndMs: render.padEndMs,
      narration: render.narration,
      activity: { bookId, chapterIndex, chunkIndex, variantKey }
    };
  }
  ...
  async _pumpManifestNow(manifest, signal = null) {
    ...
    for (const chunkIndex of candidates) {
      ...
      try {
        await this._materializeChunk(manifest, chunkIndex, signal);
      } catch (error) {
  ...
  _bookEpoch(bookId) {
    return this._bookEpochs.get(bookId) || 0;
  }

  _trackMaterialization(bookId, record) {
    let records = this._materializations.get(bookId);
    if (!records) {
      records = new Set();
      this._materializations.set(bookId, records);
    }
    records.add(record);
  }

  _assertMaterializationCurrent(manifest, epoch, signal) {
    throwIfGenerationAborted(signal);
    if (
      this._bookEpoch(manifest.bookId) !== epoch ||
      this._deletedBooks.has(manifest.bookId) ||
      manifest._generation?.cancelled
    ) {
      const error = new Error('Generation intent was retired');
      error.name = 'AbortError';
      throw error;
  ...
  _releaseMaterializationClaim(jobId, activity) {
    if (typeof this.queue.cancelWhere === 'function') {
      return this.queue.cancelWhere((claim, job) =>
        job.id === jobId &&
        claim.schedulerId === this._schedulerId &&
        claim.bookId === activity.bookId &&
        claim.chapterIndex === activity.chapterIndex &&
        claim.chunkIndex === activity.chunkIndex &&
        claim.variantKey === activity.variantKey &&
        claim.origin === activity.origin &&
        claim.requestId === activity.requestId &&
        claim.sessionId === activity.sessionId
      );
    }
    return this.queue.cancel(jobId) ? 1 : 0;
  }

  _materializeChunk(manifest, chunkIndex, signal = null) {
    const epoch = this._bookEpoch(manifest.bookId);
    const controller = new AbortController();
    const onAbort = () => controller.abort();
    signal?.addEventListener('abort', onAbort, { once: true });
    if (signal?.aborted) controller.abort();
    const record = { controller, promise: null };
    this._trackMaterialization(manifest.bookId, record);
    const promise = this._materializeChunkNow(manifest, chunkIndex, controller.signal, epoch)
      .finally(() => {
        signal?.removeEventListener('abort', onAbort);
        const records = this._materializations.get(manifest.bookId);
        records?.delete(record);
        if (records?.size === 0) this._materializations.delete(manifest.bookId);
      });
    record.promise = promise;
    return promise;
  }

  async _materializeChunkNow(manifest, chunkIndex, signal, epoch) {
    const plan = manifest._generation;
    const chunk = manifest.chunks[chunkIndex];
    if (!plan || !chunk || chunk.status !== STATUS.PENDING) return false;
    this._assertMaterializationCurrent(manifest, epoch, signal);
    ...
    try {
      ...
      this._releaseMaterializationClaim(jobId, activity);
      throw error;
    }
   ...
   */
  async concatenateChunks(bookId, chapterIndex, options = {}) {
    ...
    if (manifest.chunks.length === 1) {
      ...
      try {
        await pipeline(
          fs.createReadStream(manifest.chunks[0].path),
          fs.createWriteStream(singlePart, { flags: 'wx' }),
          { signal }
        );
        throwIfGenerationAborted(signal);
        await fsp.rename(singlePart, outputPath);
      } finally {
        await fsp.unlink(singlePart).catch(() => {});
      }
      await this._notifyChapterConcatenated(bookId, chapterIndex, outputPath);
      return outputPath;
    }
    ...
    try {
      ...
      await runAbortableCommand('ffmpeg', [
        '-hide_banner',
        '-loglevel', 'error',
        '-xerror',
        '-y',
  ...
  cancelBook(bookId) {
    ...
    this._bookEpochs.set(bookId, this._bookEpoch(bookId) + 1);
    this._deletedBooks.add(bookId);
    let cancelled = 0;
    for (const worker of this._recoveryWorkers.values()) {
      cancelled += worker.cancelBook(bookId);
    }
    for (const record of this._materializations.get(bookId) || []) {
      record.controller.abort();
    }
    if (typeof this.queue.cancelWhere === 'function') {
      cancelled += this.queue.cancelWhere(claim =>
        claim.bookId === bookId && claim.schedulerId === this._schedulerId
      );
    }
    for (const [key, manifest] of this.manifests.entries()) {
      ...
      if (typeof this.queue.cancelWhere !== 'function') {
        for (const chunk of manifest.chunks || []) {
          if (chunk.jobId && this.queue.cancel(chunk.jobId)) cancelled++;
        }
      }
  ...
  async waitForIdle(bookId) {
    while (this._materializations.has(bookId)) {
      await Promise.all([...(this._materializations.get(bookId) || [])]
        .map(record => record.promise?.catch(() => {})));
      await new Promise(resolve => setImmediate(resolve));
    }
    await Promise.all([...this._recoveryWorkers.values()]
      .map(worker => worker.waitForIdle?.(bookId)));
  }
