...
class TTSQueue extends EventEmitter {
  ...
  async enqueue({
    ...
    // Chunk output paths are content-addressed, so an existing artifact is
    // already the right audio for this job. Callers that mean "render this
    // again" (an explicit retry over a suspect file) leave this off.
    ...
    if (Number.isInteger(materializationLimit) && materializationLimit > 0 && activity) {
