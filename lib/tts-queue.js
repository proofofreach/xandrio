...
class TTSQueue extends EventEmitter {
  ...
  async enqueue({
    ...
    const artifactReady = artifact
      ? await this._resolveArtifact(artifact)
      : false;
  ...
  async _resolveArtifact(artifact) {
    try {
      if (await outputAlreadyRendered(artifact.outputPath)) {
        await this.artifactCache.publish(artifact);
        return true;
      }
      return await this.artifactCache.resolve(artifact);
    } catch (error) {
  ...
  async indexRenderedOutput(params) {
    ...
    try {
      ...
      return this.artifactCache.publish(artifact);
    } catch (error) {
  ...
  async _processJob(job) {
    ...
    try {
      ...
      if (job.reuseExistingOutput && await outputAlreadyRendered(job.outputPath)) {
        if (job.artifact) await this.artifactCache.publish(job.artifact).catch(error => {
