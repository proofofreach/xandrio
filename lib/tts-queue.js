...
const { NarrationArtifactCache } = require('./narration-artifact-cache');
const { narrationRenderRecipe } = require('./narration-render-recipe');

// Unique temp path per writer so concurrent generations for the same
// outputPath never truncate each other's temp file (Sol agree #2).
let partCounter = 0;
function uniquePartPath(outputPath, suffix = '.part') {
  const rand = crypto.randomBytes(4).toString('hex');
  return `${outputPath}.${process.pid}.${rand}.${++partCounter}${suffix}`;
}
...
/**
 * Whether a job's output has already been rendered. Zero-length counts as
 * absent: a truncated file from an interrupted write is not reusable audio.
 */
async function outputAlreadyRendered(outputPath) {
...
class TTSQueue extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    this.foregroundBookTtlMs = Math.max(1000, Number(options.foregroundBookTtlMs) || 45000);
    ...
    this.artifactCache = options.artifactCache || (
      this.cacheDir ? new NarrationArtifactCache(this.cacheDir) : null
    );
    ...
    this._foregroundBookId = '';
    this._foregroundBookExpiresAt = 0;
    // Book ids in the order the reader asked for, most wanted first. Empty by
    // default, and consulted only to break ties *within* one priority band —
    // see _takeNextQueuedJob(). It never promotes speculative work above
    // interactive work, so a reordered backlog cannot delay live playback.
    this._bookOrder = [];
   ...
   * @param {AbortSignal} [params.signal] - Cancels cache resolution before a job exists
  ...
  async enqueue({
    ...
    narration = null,
    activity = null,
    materializationLimit = null,
    // Chunk output paths are variant-scoped, so an existing artifact is
    // already the right audio for this job. The canonical artifact cache adds
    // the finer per-render fingerprint used across compatible variants.
    // Callers that mean "render this again" leave this off.
    ...
    if (reuseExistingOutput) {
      try {
        artifact = this._artifactDescriptor({
          text,
          outputPath,
          language,
          voice,
        ...
        console.warn(`[tts-queue] Selective narration cache recipe failed: ${error.message}`);
      }
    }
    const artifactReady = artifact
      ? await this._resolveArtifact(artifact, signal)
      : false;
    throwIfAborted(signal);
    const racedId = outputPath ? this._jobsByOutputPath.get(outputPath) : null;
    const raced = racedId ? this._jobs.get(racedId) : null;
    if (raced && (raced.status === 'queued' || raced.status === 'generating')) {
      this._addActivityClaim(raced, activity, priority);
      if (this._isHigherPriority(priority, raced.priority)) {
        this.prioritize(raced.id, priority);
      }
      return raced.id;
    }
    if (racedId) this._jobsByOutputPath.delete(outputPath);
    throwIfAborted(signal);
    if (!artifactReady && Number.isInteger(materializationLimit) && materializationLimit > 0 && activity) {
      const liveForChapter = [...this._active.values(), ...this._queue].filter(job =>
        this._activityClaims(job).some(claim =>
          claim.bookId === activity.bookId &&
          claim.chapterIndex === activity.chapterIndex &&
          claim.variantKey === activity.variantKey
        )
      ).length;
      if (liveForChapter >= materializationLimit) {
        const error = new Error('Chapter materialization window is full');
        error.code = 'TTS_MATERIALIZATION_LIMIT';
        throw error;
      }
    }
  ...
  getQueueStatus() {
    ...
    if (this.artifactCache?.snapshot) status.artifacts = this.artifactCache.snapshot();
    return status;
  }

  _artifactDescriptor({ text, outputPath, language, voice, padEndMs, narration, activity }) {
    if (!this.artifactCache || !outputPath || !activity || typeof activity.bookId !== 'string' ||
      !Number.isInteger(activity.chapterIndex) || activity.chapterIndex < 0) {
      return null;
    }
    const userVoice = voice || getSettings().voice || this.defaultVoice;
    const adapter = this.engineAdapters.resolve(userVoice);
    const prepared = narrationRenderRecipe({
      adapter,
      text,
      outputPath,
      language,
      voice: userVoice,
      variantKey: activity.variantKey || null,
      padEndMs,
      narration
    });
    return {
  ...
  async _resolveArtifact(artifact, signal) {
    try {
      if (await this.artifactCache.reuseExisting(artifact, { signal })) return true;
      return await this.artifactCache.resolve(artifact, { signal });
    } catch (error) {
      if (error?.name === 'AbortError') throw error;
      console.warn(`[tts-queue] Selective narration cache lookup failed: ${error.message}`);
      return false;
    }
  }

  async reuseRenderedOutput(params) {
    try {
      const artifact = this._artifactDescriptor(params);
      if (!artifact) return outputAlreadyRendered(params?.outputPath);
      return this._resolveArtifact(artifact);
    } catch (error) {
      console.warn(`[tts-queue] Selective narration cache recipe failed: ${error.message}`);
      return outputAlreadyRendered(params?.outputPath);
    }
  }

  async indexRenderedOutput(params) {
    if (!(await outputAlreadyRendered(params?.outputPath))) return false;
    try {
      const artifact = this._artifactDescriptor(params);
      if (!artifact) return false;
      return this.artifactCache.publishLegacy(artifact);
    } catch (error) {
      console.warn(`[tts-queue] Selective narration cache indexing failed: ${error.message}`);
      return false;
    }
  }

  renderedOutputFingerprint(params) {
    return this._artifactDescriptor(params)?.fingerprint || null;
  }

  async renderedOutputIdentity(params) {
    const artifact = this._artifactDescriptor(params);
    if (!artifact) return null;
    return this.artifactCache.inspect(artifact);
  }

  invalidateRenderedOutput({ bookId, chapterIndex, outputPath }) {
    if (!this.artifactCache || !outputPath) return Promise.resolve(false);
    return this.artifactCache.invalidate({ bookId, chapterIndex, outputPath }).catch(error => {
      console.warn(`[tts-queue] Selective narration cache invalidation failed: ${error.message}`);
      return false;
    });
  }
  ...
  getQueueActivity({ bookIds = null } = {}) {
    ...
    return {
      ...
      books: [...books.values()]
        ...
        // Explicit order first, so a book the reader moved visibly moves. Books
        // they have not placed keep the previous "busiest first" arrangement,
        // behind the ones they have.
        .sort((a, b) => (
          (this._bookOrderRank(a.bookId) - this._bookOrderRank(b.bookId))
          || (b.active - a.active)
          || (b.queued - a.queued)
  ...
  /**
   * Whether a job with background priority is *actively generating* right now.
   *
   * Diagnostics only — nothing schedules on this. Playback instrumentation uses
   * it to attribute a slow first segment: waiting behind background generation
   * is a priority inversion worth measuring, whereas waiting behind foreground
   * work (often the very chapter being requested) is not. Queued background work
   * is excluded because it holds no engine and blocks nobody.
   * @returns {boolean}
   */
  hasActiveBackgroundWork() {
    for (const job of this._active.values()) {
      if (job.priority === GENERATION_PRIORITY.BACKGROUND) return true;
    }
    return false;
  }
  ...
  cancel(jobId) {
    ...
    this.emit('settled', { jobId, status: 'cancelled' });
  ...
  getActivityClaims(jobId) {
    const job = this._jobs.get(jobId);
    if (!job || (job.status !== 'queued' && job.status !== 'generating')) return [];
    return this._activityClaims(job).map(claim => ({ ...claim }));
  }
  ...
  /**
   * Move every queued job claimed by a book to the front of its existing
   * scheduling band. This preserves interactive/download/background fairness
   * while making the foreground title win over peers with the same intent.
   *
   * @param {string} bookId
   * @returns {number} number of queued jobs moved
   */
  prioritizeBook(bookId) {
    if (typeof bookId !== 'string' || !bookId) return 0;
    this._foregroundBookId = bookId;
    this._foregroundBookExpiresAt = this.now() + this.foregroundBookTtlMs;
    const matches = this._queue.filter(job =>
      this._activityClaims(job).some(claim => claim.bookId === bookId)
    );
    let moved = 0;
    for (let index = matches.length - 1; index >= 0; index -= 1) {
      if (this.prioritize(matches[index].id, matches[index].priority)) moved += 1;
    }
    return moved;
  }

  foregroundBookId() {
    if (!this._foregroundBookId) return '';
    if (this.now() < this._foregroundBookExpiresAt) return this._foregroundBookId;
    this._foregroundBookId = '';
    this._foregroundBookExpiresAt = 0;
    return '';
  }
  ...
  /**
   * Replace the reader's explicit book order. Unknown or duplicate ids are
   * dropped; books absent from the list keep their arrival order behind every
   * book that is in it.
   *
   * @param {Iterable<string>} bookIds
   * @returns {string[]} the order as stored
   */
  setBookOrder(bookIds) {
    const seen = new Set();
    const order = [];
    for (const bookId of bookIds || []) {
      if (typeof bookId !== 'string' || !bookId || seen.has(bookId)) continue;
      seen.add(bookId);
      order.push(bookId);
    }
    this._bookOrder = order;
    // Selection reads the order at pick time, so nothing needs re-sorting —
    // but a worker idling on an empty slot should reconsider immediately.
    process.nextTick(() => this._drain());
  ...
  /**
   * Move one book one place towards the front ('up') or the back ('down') of
   * the explicit order.
   *
   * @param {string} bookId
   * @param {'up'|'down'} direction
   * @param {Iterable<string>} [seedOrder] order to establish first, so the move
   *   is relative to what the reader is actually looking at rather than to a
   *   list this queue has never been told about.
   * @returns {string[]|null} the new order, or null when the move is not possible
   */
  moveBook(bookId, direction, seedOrder = null) {
    if (typeof bookId !== 'string' || !bookId) return null;
    if (direction !== 'up' && direction !== 'down') return null;
    if (seedOrder) this.setBookOrder(seedOrder);
    const index = this._bookOrder.indexOf(bookId);
    if (index < 0) return null;
    const target = direction === 'up' ? index - 1 : index + 1;
    // Already at the end it is being moved towards.
    if (target < 0 || target >= this._bookOrder.length) return null;
    const order = [...this._bookOrder];
    [order[index], order[target]] = [order[target], order[index]];
    return this.setBookOrder(order);
  }

  /** Position of a book in the explicit order; unordered books sort last. */
  _bookOrderRank(bookId) {
    if (this._bookOrder.length === 0) return Number.MAX_SAFE_INTEGER;
    const index = this._bookOrder.indexOf(bookId);
    return index < 0 ? Number.MAX_SAFE_INTEGER : index;
  }

  /** Best (lowest) explicit-order rank across every book a job serves. */
  _jobBookOrderRank(job) {
    if (this._bookOrder.length === 0) return Number.MAX_SAFE_INTEGER;
    let rank = Number.MAX_SAFE_INTEGER;
    for (const claim of this._activityClaims(job)) {
      const claimRank = this._bookOrderRank(claim.bookId);
      if (claimRank < rank) rank = claimRank;
    }
    return rank;
  }
  ...
  _takeNextQueuedJob() {
    const foregroundBookId = this.foregroundBookId();
    const firstOf = priority => {
      if (priority === GENERATION_PRIORITY.DOWNLOAD && foregroundBookId) {
        const preferred = this._queue.find(job =>
          job.priority === priority &&
          this._activityClaims(job).some(claim => claim.bookId === foregroundBookId)
        );
        if (preferred) return preferred;
      }
      // The reader's explicit order breaks ties inside this band only. With no
      // order set every rank is equal and this is the original FIFO find().
      if (this._bookOrder.length > 0) {
        let best = null;
        let bestRank = Number.MAX_SAFE_INTEGER;
        for (const job of this._queue) {
          if (job.priority !== priority) continue;
          const rank = this._jobBookOrderRank(job);
          // Strictly less than: equal ranks keep the earlier arrival, so FIFO
          // still decides between two chapters of the same book.
      ...
      return this._queue.find(job => job.priority === priority) || null;
    };
    ...
    if (!selected) {
      ...
      if (lookahead && download) {
        // Age the guard off the oldest waiting download, not off whichever one
        // preference happened to select. They were the same job back when this
        // band was strictly FIFO; a reordered — or front-of-band promoted —
        // queue can otherwise hide a genuinely starving download behind a
        // newer, more-wanted one.
        const oldestDownloadAt = this._queue.reduce(
          (oldest, job) => (job.priority === GENERATION_PRIORITY.DOWNLOAD
            ? Math.min(oldest, Number(job.createdAt) || this.now())
            : oldest),
          Infinity
        );
        selected = this._consecutiveLookahead >= this.lookaheadBurst ||
          this.now() - oldestDownloadAt >= this.downloadMaxWaitMs
          ? download
          : lookahead;
      } else {
    ...
    // Everything left — background, and any priority not named above. The queue
    // is kept in priority order, so its head names the top remaining band;
    // running firstOf() over that band applies the reader's order there too.
    // With no order set firstOf() returns that same head, unchanged.
    if (!selected && this._queue.length > 0) selected = firstOf(this._queue[0].priority);
   ...
   */
  /**
   * Settle a job as complete. Shared by the generated and the reused-artifact
   * paths so a reused output is indistinguishable to every listener — the
   * manifest still learns its chunk is ready, waiters still resolve.
   * @private
   */
  _completeJob(job, { reused = false } = {}) {
    job.status = 'complete';
    ...
    this.emit('settled', { jobId: job.id, status: 'complete' });
  ...
  async _processJob(job) {
    ...
    try {
      // Last line of defence against duplicate synthesis. The manifest layer
      // adopts rendered chunks before it enqueues, but a job can also be
      // enqueued while another one for the same path is finishing, or be
      // waiting in the queue when something else renders its output. TTS is by
      // far the most expensive thing this server does; one stat is not.
      const reusableOutput = job.reuseExistingOutput && (job.artifact
        ? await this.artifactCache.reuseExisting(job.artifact, {
            signal: job._controller.signal
          })
        : await outputAlreadyRendered(job.outputPath));
      throwIfAborted(job._controller.signal);
      if (reusableOutput) {
        this._completeJob(job, { reused: true });
        return;
      }
      ...
      if (job.artifact) {
        try {
          await this.artifactCache.publishVerified(job.artifact, null, {
            signal: job._controller.signal
          });
        } catch (error) {
          if (error?.name === 'AbortError') throw error;
          console.warn(`[tts-queue] Selective narration cache publish failed: ${error.message}`);
        }
      }
      throwIfAborted(job._controller.signal);
      ...
      if (job._cancelRequested) {
        ...
        this.emit('settled', { jobId: job.id, status: 'cancelled' });
        return;
      }
      job.status = 'error';
      job._error = err;
      ...
      // NOTE: must NOT use the special 'error' event name — Node throws if
      // an 'error' event has no listener. 'job-error' is a plain event.
      this.emit('job-error', {
        jobId: job.id,
        error: err,
        active: this._active.size,
      ...
      this.emit('settled', { jobId: job.id, status: 'error' });
  ...
  async _generateTTS(
    ...
    return render.adapter.generate({
      text: render.text,
      outputPath,
  ...
  async _generateEdgeTTSOnce({ text, outputPath, language, voice, padEndMs = 0, gainDb = 0, signal = null }) {
    ...
    // Generate to a unique temp path and rename so an interrupted run can
    // never leave a truncated file at outputPath (a size>0 file there is
    // trusted as a ready chunk by ChunkedTTS), and concurrent writers for
    // the same outputPath cannot truncate each other.
    const partPath = uniquePartPath(outputPath);
    try {
  ...
  /** Write an audio buffer to outputPath atomically (unique temp + rename). */
  ...
  async _convertAudioBufferToOutput(audioBuffer, inputFormat, outputPath, padEndMs = 0, gainDb = 0, signal = null, outputFormat = null) {
    ...
    const partPath = uniquePartPath(outputPath, `.part.${finalFormat}`);
    throwIfAborted(signal);
    try {
  ...
  async _cleanupOutputArtifacts(outputPath) {
    // Best-effort sweep of legacy deterministic temps plus common variants.
    // Per-write unique temps are unlinked by their own writer; this remains
    // for crash leftovers and older files.
    await Promise.all([
      outputPath,
      `${outputPath}.part`,
      `${outputPath}.part.mp3`,
