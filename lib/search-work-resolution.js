...
function publisherTitleAliases(title, publisher, author) {
  ...
  const withoutContentForms = String(title || '').replace(BIBLIOGRAPHIC_FORM_PARENS, ' ');
  if (withoutContentForms !== String(title || '')) {
  ...
  const filename = String(title || '').match(/^(.+?)_+([^_]+)$/u);
  ...
  const subtitle = String(title || '').match(/^(.+?)\s*:\s*\S/u);
  if (subtitle) {
