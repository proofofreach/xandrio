...
const TITLE_METADATA_WORDS = new Set([
...
function publisherTitleAliases(title, publisher, author) {
  ...
  const remainder = catalogPrefixRemainder(scanTitle);
  const remainderCanonical = remainder ? canonicalWorkTitle(remainder) : '';
  if (remainderCanonical && remainderCanonical !== canonical) {
    aliases.push({ title: remainderCanonical, kind: 'catalog-prefix' });
  }
  const subtitleScan = remainder || scanTitle;
  const subtitle = subtitleScan.match(/^(.+?)\s*:\s*\S/u);
  if (subtitle) {
...
function factsFor(edition) {
  ...
  const title = canonicalWorkTitle(edition.title);
  const authorAsTitle = canonicalWorkTitle(edition.author);
  const authorTokens = authorAsTitle.split(' ').filter(Boolean);
  const authorLooksTitleLike = authorTokens.length > 5 ||
    authorTokens.some(token => TITLE_METADATA_WORDS.has(token) || /\d/u.test(token));
  return {
    edition,
    editionIdentity: editionIdentity(edition),
    title,
    authorAsTitle,
    titleShapedCreator: Boolean(authorLooksTitleLike && (
      title === authorAsTitle ||
      title.endsWith(` ${authorAsTitle}`)
    )),
    ...
    contributorProfiles: contributorIdentityProfiles(edition.author),
    ...
    qualifiers: contentQualifiers(catalogPrefixRemainder(edition.title) || edition.title)
  };
}

function compatibleFactPartition(left, right) {
  if (left.language && right.language && left.language !== right.language) return false;
  if (left.qualifiers.volume !== right.qualifiers.volume) return false;
  if (left.qualifiers.collectionScope !== right.qualifiers.collectionScope) return false;
  if (left.qualifiers.derivative !== right.qualifiers.derivative) return false;
  return left.qualifiers.adapted === right.qualifiers.adapted;
}

function creatorAnchorMatches(malformed, candidate) {
  if (!candidate.creator || candidate.titleShapedCreator ||
      !compatibleFactPartition(malformed, candidate)) return false;
  if (malformed.title === candidate.title) return true;
  if (!malformed.authorAsTitle || malformed.authorAsTitle !== candidate.title ||
      !malformed.title.endsWith(` ${candidate.title}`)) return false;
  const prefix = malformed.title.slice(0, -(candidate.title.length + 1)).trim();
  return creatorProfilesCompatible(
    candidate.creatorProfiles,
    creatorIdentityProfiles(prefix)
  );
}

function repairTitleShapedCreators(facts) {
  for (const malformed of facts.filter(fact => fact.titleShapedCreator)) {
    const candidates = facts
      .filter(candidate => candidate !== malformed && creatorAnchorMatches(malformed, candidate))
      .sort((left, right) =>
        Number(Boolean(right.openLibraryWorkKey)) - Number(Boolean(left.openLibraryWorkKey)) ||
        left.creatorDisplay.length - right.creatorDisplay.length ||
        left.editionIdentity.localeCompare(right.editionIdentity)
      );
    if (!candidates.length) continue;
    const anchor = candidates[0];
    if (candidates.some(candidate =>
      !creatorProfilesCompatible(anchor.creatorProfiles, candidate.creatorProfiles)
    )) continue;
    const hasTrustedAnchor = candidates.some(candidate =>
      candidate.openLibraryWorkKey || candidate.authorityCreator
    );
    if (candidates.length < 2 && !hasTrustedAnchor) continue;

    const titlePrefix = malformed.authorAsTitle && malformed.title.endsWith(` ${malformed.authorAsTitle}`)
      ? malformed.title.slice(0, -(malformed.authorAsTitle.length + 1)).trim()
      : '';
    const displayAnchor = candidates.find(candidate =>
      titlePrefix && candidate.creatorProfiles.some(profile => profile === titlePrefix)
    ) || anchor;
    malformed.creator = displayAnchor.creator;
    malformed.creatorDisplay = displayAnchor.creatorDisplay;
    malformed.creatorProfiles = displayAnchor.creatorProfiles.slice();
    malformed.titleAliases = publisherTitleAliases(
      malformed.edition.title,
      malformed.edition.publisher,
      displayAnchor.creatorDisplay
    );
  }
  return facts;
}

function fallbackFactIdentity(fact) {
  if (!fact.title) return fallbackWorkIdentity(fact.edition);
  return `${fact.title}|${fact.creator || 'unknown'}`;
}
...
function aggregateGroup(baseIdentity, facts) {
  return {
    ...
    contributorProfiles: facts.flatMap(fact => fact.contributorProfiles),
...
function initialGroups(results) {
  const facts = repairTitleShapedCreators(results.map(factsFor));
  const fallbackToOpenLibrary = new Map();
  for (const fact of facts) {
    const fallback = fallbackFactIdentity(fact);
...
function hardConflict(left, right, options = {}) {
  ...
  if (left.openLibraryKeys.size && right.openLibraryKeys.size && setIntersection(left.openLibraryKeys, right.openLibraryKeys).length === 0) {
    ...
    if (!recognizedEditionAlias && !sharedTrustedAuthorityIdentity(left, right) &&
        !subtitleEvidence(left, right) && !subtitleExtensionEvidence(left, right)) {
      return 'openlibrary-conflict';
    }
  }
...
function identityTitles(group) {
  const titles = new Set(group.titles);
  for (const alias of group.aliases) {
    if (alias.kind === 'catalog-prefix' && alias.title) titles.add(alias.title);
  }
  return titles;
}
...
function subtitleEvidence(left, right) {
  const candidates = [];
  const rightTitles = identityTitles(right);
  const leftTitles = identityTitles(left);
  for (const alias of left.aliases.filter(candidate => candidate.kind === 'subtitle-base')) {
    if (rightTitles.has(alias.title)) candidates.push(alias.title);
  }
  for (const alias of right.aliases.filter(candidate => candidate.kind === 'subtitle-base')) {
    if (leftTitles.has(alias.title)) candidates.push(alias.title);
  }
...
// Providers do not agree on what "first author" means. One lists the publisher
// ahead of every person ("W. W. Norton;Company.;Hoerni, Ulrich;Jung, Carl
// Gustav;…"), another sorts contributors alphabetically so the preface author
// leads ("Hoerni, Ulrich; Jung, Carl Gustav;…"). Taking the primary creator as
// "whoever was listed first" then forks one work into a row per provider.
//
// Widening identity to any shared contributor would merge unrelated works that
// share a title, so require the stronger asymmetric signal: under an identical
// canonical title, the primary creator one group settled on is a credited
// contributor of the other.
function contributorCreatorEvidence(left, right) {
  const sharedTitles = setIntersection(left.titles, right.titles);
  if (!sharedTitles.length) return null;
  const creditedBy = (group, other) => other.contributorProfiles.some(profiles =>
    creatorProfilesCompatible(group.creatorProfiles, profiles));
  if (!creditedBy(left, right) && !creditedBy(right, left)) return null;
  return {
    title: sharedTitles.sort()[0],
    method: 'contributor-title-author',
    confidence: 'corroborated',
    evidence: ['canonical-title', 'credited-contributor']
  };
}
...
function matchGroups(left, right) {
  ...
  const contributorCreator = contributorCreatorEvidence(left, right);
  const conflict = hardConflict(left, right);
  if (conflict && !(conflict === 'creator-conflict' &&
      (creatorTypo || authorityBibliographic || contributorCreator))) {
  ...
  if (contributorCreator) return { matched: true, ...contributorCreator };
...
// Merging two clusters requires every cross pair to agree, so a group whose
// primary creator is a mis-ordered contributor must also reconcile with cluster
// members that spell the shared author differently ("c g jung" beside "carl
// gustav jung"). One member crediting both sides is enough to anchor it.
function anchoredContributorCreatorEvidence(left, right, clusterGroups) {
  const anchor = clusterGroups.find(group =>
    group !== left && group !== right &&
    contributorCreatorEvidence(group, left) &&
    contributorCreatorEvidence(group, right)
  );
  if (!anchor) return null;
  return {
    matched: true,
    title: contributorCreatorEvidence(anchor, left).title,
    method: 'contributor-title-author',
    confidence: 'corroborated',
    evidence: ['credited-contributor-anchor', 'canonical-title']
  };
}
...
function clusterPairMatch(left, right, clusterGroups, resolutionMode) {
  ...
  return anchoredTrustedAuthorityEvidence(left, right, clusterGroups) ||
    ...
    anchoredContributorCreatorEvidence(left, right, clusterGroups) ||
    direct;
}
