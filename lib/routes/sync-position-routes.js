...
const { badRequest, notFound, invalidBookId } = require('../http-error');
...
app.post('/api/position', async (req, res) => {
  try {
    ...
    if (!isSafeBookId(bookId) || parsedChapterIndex === null || !Number.isFinite(parsedTimestamp) || parsedTimestamp < 0) {
      return badRequest(res, 'Invalid playback position');
    }
    if (chunkIndex !== undefined && parsedChunkIndex === null) {
      return badRequest(res, 'Invalid playback chunk');
    }
    if (chunkTime !== undefined && (!Number.isFinite(parsedChunkTime) || parsedChunkTime < 0)) {
      return badRequest(res, 'Invalid playback chunk time');
    }
    if (characterOffset !== undefined && parsedCharacterOffset === null) {
      return badRequest(res, 'Invalid playback character offset');
    }
    ...
    if (write.status === 404) return notFound(res, 'Book not found');
...
app.post('/api/positions/batch', async (req, res) => {
  try {
    ...
    if (bookIds.length > 500) {
      return badRequest(res, 'Too many book identifiers');
    }
    const safeIds = [];
