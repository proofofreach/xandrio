...
function registerPreferencesRoutes(app, {
  ...
  async function generateVoiceSample(voiceId, samplePath) {
    ...
    try {
      if (voiceId.startsWith('kokoro:')) {
        ...
        await tempQueue._generateKokoroTTS(sampleText, tempPath, TTSQueue.getKokoroLanguage('en', kokoroVoice), kokoroVoice);
        ...
        await tempQueue._generateChatterboxTTS(sampleText, tempPath, voiceId.slice('chatterbox:'.length));
      } else {
      ...
      // Rename into place per-request-unique -> final so two concurrent
      // generations can never share (and truncate) the same partial file.
      ...
      if (!cached) {
        ...
        if (voiceId.startsWith('kokoro:') || voiceId.startsWith('chatterbox:')) {
          ...
          if (status.engines?.[engine]?.up === false) {
            return res.status(503).json({ error: 'Narration engine is unavailable', code: 'ENGINE_UNAVAILABLE' });
          }
        }
        if (!voiceSampleInFlight.has(voiceId)) {
          voiceSampleInFlight.set(
            voiceId,
            generateVoiceSample(voiceId, samplePath).finally(() => voiceSampleInFlight.delete(voiceId))
          );
        }
        await voiceSampleInFlight.get(voiceId);
      }
    ...
    try {
      ...
      if (chatterboxVoicesEnabled === false) {
        return res.status(403).json({
          error: 'Voice cloning is not available on this instance (Chatterbox is not an enabled voice provider).',
      ...
      let conflict = false;
      await jsonStore.update(customVoicesFile, registry => {
  ...
  app.delete('/api/voices/clone/:id', requireAdmin, async (req, res) => {
    try {
      ...
      if (!found) return res.status(404).json({ error: 'Voice not found' });
