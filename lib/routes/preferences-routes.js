...
function registerPreferencesRoutes(app, {
  ...
  app.post('/api/annas/configure', async (req, res) => {
    try {
      ...
      if (existing.keySource === 'environment') {
        return res.status(409).json({
          error: 'Anna’s Archive key is managed by ANNAS_SECRET_KEY. Update the server environment and restart Xandrio.',
          code: 'ANNAS_KEY_ENVIRONMENT_MANAGED'
        });
      }
      ...
      res.json({ success: true, replaced: Boolean(existing.secretKey), updatedAt });
    } catch (err) {
  ...
  app.delete('/api/annas/configure', async (req, res) => {
    try {
      const current = getAnnasConfig();
      if (current.keySource === 'environment') {
        return res.status(409).json({
          error: 'Anna’s Archive key is managed by ANNAS_SECRET_KEY. Remove it from the server environment and restart Xandrio.',
          code: 'ANNAS_KEY_ENVIRONMENT_MANAGED',
          configured: true,
