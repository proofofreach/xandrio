...
function registerBookGuideRoutes(app, {
  ...
  app.get('/api/book/:bookId/guide/narration/:sectionId/audio', async (req, res) => {
    if (!validBook(req, res)) return;
    try {
  ...
  app.get('/api/book-guides/provider/connection', requireAdmin, async (_req, res) => {
    try {
  ...
  app.post('/api/book-guides/provider/login', requireAdmin, async (_req, res) => {
    try {
      res.set('Cache-Control', 'private, no-store');
  ...
  app.delete('/api/book-guides/provider/connection', requireAdmin, async (_req, res) => {
    try {
      res.set('Cache-Control', 'private, no-store');
