...
const { badRequest, notFound, conflict, invalidBookId, sendError } = require('../http-error');
...
app.get('/api/voice-cache/:bookId/:chapterIndex', async (req, res) => {
  try {
    ...
    if (!chapter) {
      return notFound(res, 'Chapter not found');
    }
...
app.post('/api/premium-prep/:bookId/start', async (req, res) => {
  try {
    ...
    if (!isPremiumVoiceActive()) {
      return conflict(res, 'Active voice has no premium tier');
    }
...
app.get('/api/queue/status', async (req, res) => {
  try {
    ...
    // Import warmup persists a terminal error on the book record, while the
    // generation queue only reports pending work. Include those errors here so
    // an empty queue cannot be mistaken for successful audio preparation.
    const activeBookIds = new Set(activity.books.map(item => item.bookId));
    const failedBooks = await Promise.all([...relevantBookIds].map(async bookId => {
      const book = books[bookId];
      if (!book || book.audioGenerationState !== 'error' || activeBookIds.has(bookId)) return null;
      const chapters = await getChaptersCached(book.path).catch(() => []);
      const preferredIndex = typeof findPreferredAudioStartChapterIndex === 'function'
        ? findPreferredAudioStartChapterIndex(chapters)
        : -1;
      return {
        id: bookId,
        title: book.title || 'Untitled',
        ...
        chapters: [],
        failed: true,
        error: String(book.audioGenerationError || 'Audio preparation failed'),
        retryChapterIndex: Number.isInteger(preferredIndex) && preferredIndex >= 0 ? preferredIndex : null
      };
    }));
    res.json({
      active: activity.active,
      queued: activity.queued,
      books: [...activityBooks.filter(Boolean), ...failedBooks.filter(Boolean)]
    });
  } catch (err) {
...
app.post('/api/queue/order', async (req, res) => {
  try {
    ...
    if (direction !== 'up' && direction !== 'down') {
      return badRequest(res, "Direction must be 'up' or 'down'");
    }
    const userId = userIdFor(req);
    ...
    if (!order && !movedPreparation) {
      ...
      return sendError(res, 409, 'This title cannot move any further', 'QUEUE_ORDER_UNCHANGED', { order: ttsQueue.bookOrder() });
    }
