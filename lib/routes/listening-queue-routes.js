...
function registerListeningQueueRoutes(app, {
  ...
  app.get('/api/listening-queue', async (req, res) => {
    try {
      ...
      console.error('Listening queue load failed:', error);
      res.status(500).json({ error: 'Failed to load listening queue' });
    }
  });
  ...
  app.put('/api/listening-queue', async (req, res) => {
    try {
      ...
      console.error('Listening queue save failed:', error);
      res.status(500).json({ error: 'Failed to save listening queue' });
    }
  });
  ...
  app.post('/api/listening-queue/items', async (req, res) => {
    try {
      ...
      console.error('Listening queue update failed:', error);
      res.status(500).json({ error: 'Failed to update listening queue' });
    }
  });
  ...
  app.patch('/api/listening-queue/items/:bookId', async (req, res) => {
    try {
      ...
      if (!isSafeBookId(bookId) || toIndex === null) return res.status(400).json({ error: 'Invalid queue move' });
      const userId = userIdFor(req);
      let queue;
      await updateJSON(listeningQueueFile, data => {
        const store = normalizeStore(data);
      ...
      console.error('Listening queue move failed:', error);
      res.status(500).json({ error: 'Failed to move queue item' });
    }
  });
  ...
  app.get('/api/listening-queue/books/:bookId/settings', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to load book playback settings' });
    }
  });
  ...
  app.put('/api/listening-queue/books/:bookId/settings', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to save book playback settings' });
    }
  });
  ...
  app.post('/api/listening-queue/advance', async (req, res) => {
    try {
      ...
      if (!isSafeBookId(finishedBookId)) return res.status(400).json({ error: 'Invalid book identifier' });
      const userId = userIdFor(req);
      ...
      console.error('Listening queue advance failed:', error);
      res.status(500).json({ error: 'Failed to advance listening queue' });
    }
  });
}
