...
async function replayOfflinePreparations({
  ...
  const intents = await generationJournal.listOfflinePreparations?.() || [];
  const report = { resumedBooks: 0, resumedChapters: 0, failedBooks: [] };
  for (const intent of intents) {
    try {
      const { chapters } = await getBookChapters(intent.bookId);
      const results = await Promise.allSettled(chapters.map((_chapter, chapterIndex) =>
        playbackOrchestrator.startChapterAudio({
          bookId: intent.bookId,
          chapterIndex,
          priority: 'download'
        })
      ));
      const resumedChapters = results.filter(result => result.status === 'fulfilled').length;
      report.resumedChapters += resumedChapters;
      if (resumedChapters === chapters.length) {
        report.resumedBooks++;
      } else {
        report.failedBooks.push(intent.bookId);
      }
    } catch {
      report.failedBooks.push(intent.bookId);
    }
...
function registerPlaybackRoutes(app, {
  ...
  const aggregateOfflinePreparation = (bookId, statuses, requested = true) => {
    ...
    return {
      bookId,
      state: !requested && readyChapters < totalChapters
        ? 'not-requested'
        : errorChapters > 0
        ? 'error'
        : readyChapters === totalChapters && totalChapters > 0
          ? 'ready'
          : 'preparing',
      readyChapters,
      totalChapters,
  ...
  const sendOfflinePreparationError = (res, error) => {
    if (error?.statusCode === 400 || error?.statusCode === 404 || error?.statusCode === 503) {
  ...
  app.get('/api/offline/preparation/:bookId', async (req, res) => {
    try {
      ...
      const { chapters } = await loadOfflinePreparationTarget(bookId);
      const statuses = await Promise.all(chapters.map((_chapter, chapterIndex) =>
        playbackOrchestrator.chapterAudioStatus({ bookId, chapterIndex })
      ));
      const intent = await generationJournal.getOfflinePreparation?.(bookId);
      res.json(aggregateOfflinePreparation(bookId, statuses, Boolean(intent)));
    } catch (error) {
  ...
  app.post('/api/offline/preparation/:bookId', async (req, res) => {
    try {
      const { bookId } = req.params;
      const { chapters } = await loadOfflinePreparationTarget(bookId);
      await generationJournal.putOfflinePreparation?.({
        bookId,
      ...
      res.status(202).json(aggregateOfflinePreparation(bookId, statuses));
    } catch (error) {
      sendOfflinePreparationError(res, error);
    }
  });
