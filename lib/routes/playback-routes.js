...
function registerPlaybackRoutes(app, {
  ...
  const aggregatePlaybackRunway = (statuses, runwayChapterIndexes) => {
    ...
    const error = statuses.some(status => status?.status === 'error');
    const generating = statuses.some(status => status?.status === 'generating');
    return {
      ready,
      status: error ? 'error' : ready ? 'ready' : generating ? 'generating' : 'pending',
      readyChunks: statuses.reduce(
        (sum, status) => sum + Math.max(0, Number(status?.readyChunks) || 0),
        0
      ),
      totalChunks: statuses.reduce(
        (sum, status) => sum + Math.max(0, Number(status?.totalChunks) || 0),
        0
      ),
      servedTier: statuses.find(status => status?.servedTier)?.servedTier,
