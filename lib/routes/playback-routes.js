...
const { badRequest, notFound, forbidden, invalidBookId, unavailable, conflict, sendError } = require('../http-error');
...
function registerPlaybackRoutes(app, {
  ...
  const requirePlaybackRunway = async (res, options) => {
    ...
    sendError(res, 425, 'Playback audio is still preparing', 'PLAYBACK_RUNWAY_NOT_READY', { ready: false });
    return false;
  };
  ...
  app.post('/api/playback/foreground/:bookId', playbackControlRateLimit, async (req, res) => {
    try {
      ...
      if (!isSafeBookId(bookId)) {
        return invalidBookId(res);
      }
      if (!await canPrioritizeForegroundBook(req, bookId)) {
        return notFound(res, 'Book is not in this library');
      }
  ...
  app.get('/api/offline/preparation/:bookId/manifest', async (req, res) => {
    try {
      await loadOfflinePreparationTarget(req.params.bookId);
      if (!getOfflineTransferManifest) return unavailable(res, 'Offline transfer descriptors are unavailable');
      const manifest = await getOfflineTransferManifest(req.params.bookId);
  ...
  app.post('/api/offline/preparation/:bookId/window', playbackGenerationRateLimit, async (req, res) => {
    try {
      await loadOfflinePreparationTarget(req.params.bookId);
      if (!offlineWindowPreparation) return unavailable(res, 'Offline windows are unavailable');
      res.status(202).json(await offlineWindowPreparation.request(
        req.params.bookId, req.body?.chapterIndexes, offlinePreparationOwner(req)
      ));
    } catch (error) {
      sendOfflinePreparationError(res, error);
    }
  ...
  app.get('/__xandrio_offline__/audio/:scope/:artifactId', (_req, res) => {
    res.status(404).end();
  });
  ...
  app.post('/api/offline/notifications', playbackControlRateLimit, async (req, res) => {
    try {
      if (!offlineReadinessNotifications?.enabled) {
        return unavailable(res, 'Readiness notifications are unavailable');
      }
  ...
  app.get('/api/offline/audio/:bookId/:chapterIndex', playbackMediaRateLimit, async (req, res) => {
    try {
      ...
      if (!isSafeBookId(bookId) || chapterIndex === null) {
        return badRequest(res, 'Invalid book or chapter identifier');
      }
      if (typeof getOfflineChapterAudio !== 'function') {
        return unavailable(res, 'Offline audio packages are unavailable');
      }
      ...
      if (!prepared?.ready || !prepared.path) {
        return conflict(res, 'Offline audio is still preparing');
      }
      if (req.query?.artifact && req.query.artifact !== prepared.artifactId) {
        return conflict(res, 'Offline audio identity changed');
      }
      return await serveAudioFile(req, res, prepared.path, { identity: prepared });
    } catch (error) {
  ...
  app.get(
    ...
    try {
      ...
      if (
        ...
        return badRequest(res, 'Invalid HLS playback identifier');
      }
  ...
  app.get('/api/audio-timeline/:sessionId', playbackMediaRateLimit, (req, res) => {
    ...
    if (!sessionId) return badRequest(res, 'Invalid playback session identifier');
    const timeline = playbackOrchestrator.continuousTimeline(sessionId);
    if (!timeline) return notFound(res, 'Playback timeline not ready');
  ...
  app.get('/api/serve-chunk/:filename', (req, res) => {
    ...
    if (!redirect) {
      return forbidden(res, 'Invalid chunk filename');
    }
  ...
  // Readiness is polled once per second during cold preparation. These reads
  // do not start synthesis and must not spend the generation request budget.
  // Keep the transport flood ceiling, including for playback-runway polls.
  app.get('/api/chunks/:bookId/:chapterIndex/chapter-audio-status', playbackMediaRateLimit, async (req, res) => {
    try {
      const { bookId } = req.params;
      const chapterIndex = parseNonNegativeInteger(req.params.chapterIndex);
      if (!isSafeBookId(bookId) || chapterIndex === null) {
  ...
  app.post('/api/chunks/:bookId/:chapterIndex/prepare-chapter-audio', playbackGenerationRateLimit, async (req, res) => {
    try {
      ...
      const importWarmup = req.body?.purpose === 'import-warmup';
      if (importWarmup) {
        if (!await canPrioritizeForegroundBook(req, bookId)) {
          return notFound(res, 'Book is not in this library');
        }
        if (typeof retryImportWarmup !== 'function') {
          return unavailable(res, 'Audio preparation is unavailable');
        }
        await retryImportWarmup({ bookId, chapterIndex });
        return res.status(202).json({ bookId, chapterIndex, status: 'queued' });
      }
  ...
  app.post('/api/chunks/:bookId/:chapterIndex/:chunkIndex/prioritize', playbackGenerationRateLimit, async (req, res) => {
    try {
      ...
      if (!isSafeBookId(bookId) || chapterIndex === null || chunkIndex === null) {
        return badRequest(res, 'Invalid chunk');
      }
      ...
      try {
        ...
        if (access.status !== 'missing') {
          ...
          if (access.status === 'error') {
            return sendError(res, 500, 'Chunk generation failed', undefined, { status: 'error' });
          }
        }
        return notFound(res, 'Chunk not found');
      }
