...
function numericOrdinalToWords(digits, suffix) {
  const value = BigInt(digits.replace(/,/g, ''));
  if (expectedOrdinalSuffix(value) !== suffix.toLowerCase()) return null;
...
function prepareTtsText(text) {
  return normalizeNumericOrdinals(normalizeNarrationCasing(removeStandaloneOrnaments(text)))
    .replace(/\r\n?/g, '\n')
    .replace(/\t/g, ' ')
    .replace(/\u00a0/g, ' ')
    .replace(/\s+([,.;:!?])/g, '$1')
