...
const { numericOrdinalToWords } = require('./tts-number-words');
...
function prepareTtsText(text) {
  return normalizeNumericOrdinals(normalizeNarrationCasing(normalizeLegalCitations(removeStandaloneOrnaments(text))))
    .replace(/\r\n?/g, '\n')
    .replace(/\t/g, ' ')
    .replace(/\u00a0/g, ' ')
    .replace(/\s+([,.;:!?])/g, '$1')
