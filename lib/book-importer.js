...
function createBookImporter(dependencies) {
  const {
    ...
    // Guards a fallback candidate's id against the *existing* library, the same
    // way server.js's assertImportTargetAvailable guards the primary id before
    // import starts. Without this, probing an alternative whose id equals an
    // already-imported different book overwrites (and, if the alternative
    // loses, deletes) that other book's canonical source file — probing writes
    // straight to CACHE_DIR/<id>.<ext>, not a scratch path. Defaults to
    // always-available so callers that don't wire a library check keep today's
    // behavior.
  ...
  async function chooseCandidate(command, progress, ownership) {
    ...
    // Candidate ids resolve straight to a canonical cache path (see prepareCandidate /
    // normalizeBook), so two candidates sharing an id would probe into and delete
    // each other's file within this same import. Never let an alternative reuse the
    // primary's id or an id already queued from an earlier alternative.
    const seenCandidateIds = new Set([String(command.id)]);
    for (const alternative of alternatives) {
      if (!alternative?.id) continue;
      const alternativeId = String(alternative.id);
      if (seenCandidateIds.has(alternativeId)) continue;
      if (alternative.shouldTry && !(await alternative.shouldTry(selectedIdentity))) continue;
      if (!(await isImportTargetAvailable(alternativeId, command.addedBy))) continue;
      seenCandidateIds.add(alternativeId);
  ...
  async function materializeArtifact(prepared, command, ownership) {
    ...
    if (!recoveryProof?.proven) {
      ...
      if (!writeArtifactData) {
        ...
        // the original becomes deletable. The artifact stays owned (unlike
        // finalPath, which is about to be unlinked on purpose): until a books.json
        // record exists nothing in the app can reach the artifact, so if a later
        // import step fails, cleanup() must still be able to remove it instead of
        // orphaning it in cache/ forever. It is released once persistBook commits.
        extractedArtifact.sourceDeleted = false;
        extractedArtifact.sourcePath = finalPath;
  ...
  async function importBook(command, progress = () => {}) {
    ...
    const metadataSeed = metadata.resolveSeed(
      ...
      prepared.candidate.id || command.id
    );
    ...
    const record = {
      ...
      canRebuildChapters: Boolean(
        chapters?.sourceDocument?.pages?.length || artifact.sourceRetainedAfterExtract
      ) || undefined,
    ...
    const coverPath = await ensureBookCover(record).catch(error => {
      log.error(`Import cover fetch failed: ${error.message}`);
      return undefined;
    });
    // ensureBookCover writes CACHE_DIR/<id>_cover.jpg (and stamps it onto
    // `record`) before the duplicate-detection check below can still reject
    // the import. Registering it here, as soon as its path is known, means a
    // rejected/duplicate import's cleanup() removes the cover instead of
    // leaving it orphaned in cache/ forever.
    if (coverPath) ownership.add(coverPath);
    ...
    // The artifact is now reachable through books.json; ownership.cleanup() will
    // never run again for this import, but release it explicitly so its lifetime
    // is not implicit in "we happened not to fail after this point".
    if (artifact.extractedArtifactPath) ownership.release(artifact.extractedArtifactPath);
    if (coverPath) ownership.release(coverPath);
