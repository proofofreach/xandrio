...
const { UNUSABLE_CHAPTER_THRESHOLD } = require('./chapter-utils');
...
function createBookImporter(dependencies) {
  ...
  async function chooseCandidate(command, progress, ownership) {
    ...
    for (let index = 0; index < maxAttempts; index++) {
      ...
      try {
        ...
        const preparedVerified = prepared.quality.structureVerified !== false;
        const bestVerified = best.quality.structureVerified !== false;
        const replacesBest = (
          preparedVerified && !bestVerified
        ) || (
          preparedVerified === bestVerified &&
          (
            prepared.quality.isGoodStructure ||
  ...
  async function materializeArtifact(prepared, command, ownership) {
    ...
    const pdfExtraction = (extractedChapters || []).find(chapter => chapter?.pdfExtraction)?.pdfExtraction;
    const retainReviewSource = String(normalized.originalFormat || '').toUpperCase() === 'PDF' &&
      pdfExtraction?.status === 'review-needed';
    if (retainReviewSource) {
      ...
      prepared.validation.warnings.push(
        'Retained original PDF because its extracted chapter structure needs review'
      );
    } else {
  ...
  async function importBook(command, progress = () => {}) {
    ...
    try {
      ...
      if (
        prepared.quality.structureVerified === false ||
        prepared.quality.maxChapterSize > UNUSABLE_CHAPTER_THRESHOLD ||
        prepared.quality.contentChapters < 1
      ) {
    ...
    const record = {
      ...
      pdfReprocessable: prepared.normalized.originalFormat === 'PDF' &&
        Boolean(chapters?.sourceDocument?.pages?.length),
      ...
      ...(command.kind === 'upload' && artifact.extractedArtifact?.embeddedCover
    ...
    return {
      ...
      validation: { valid: true, warnings: importValidation.warnings },
