...
const { fromLegacyChapters, attachMutationActivations, PROCESSING_VERSION } = require('./extraction-result');
...
function createBookDocument(options = {}) {
  ...
  const chapterCacheVersion = options.chapterCacheVersion || 31;
  ...
  const chapterInflightGenerations = new Map();
  ...
  function getRebuildArtifactPath(bookPath) {
    return isXBookPath(bookPath) ? bookPath : getChapterCachePath(bookPath);
  }

  async function canRebuildChapters(bookPath) {
    if (isXBookPath(bookPath)) return xbookStore()?.canRebuildXBookArtifact(bookPath) || false;
    if (getFormatFromName(bookPath) !== 'epub') return false;
    try {
      await Promise.all([fileSystem.access(bookPath), fileSystem.access(getChapterCachePath(bookPath))]);
      return true;
    } catch { return false; }
  }

  async function planChapterRebuild(bookPath, options = {}) {
    if (isXBookPath(bookPath)) return xbookStore().planXBookRebuild(bookPath, options);
    if (!await canRebuildChapters(bookPath)) {
      const error = new Error('The retained source and chapter cache are unavailable for rebuilding');
      error.code = 'XBOOK_REPROCESS_UNSUPPORTED';
      throw error;
    }
    // Read the persisted narration directly. A cache-version migration or
    // fresh extraction here would erase the evidence needed to remap readers.
    const artifact = JSON.parse(await fileSystem.readFile(getChapterCachePath(bookPath), 'utf8'));
    if (!Number.isInteger(artifact._cacheVersion) || !Array.isArray(artifact.chapters)) {
      return { safe: false, reason: 'invalid-chapter-cache' };
    }
    const chapters = await extractChapters(bookPath);
    const transition = buildChapterTransition(artifact.chapters, chapters);
    const previousStructureKey = chapterStructureKey(artifact.chapters);
    const nextStructureKey = chapterStructureKey(chapters);
    return {
      safe: transition.safe,
      changed: transition.safe && previousStructureKey !== nextStructureKey,
      artifact,
      candidate: { _cacheVersion: chapterCacheVersion, processingVersion: PROCESSING_VERSION, chapters },
      transition,
  ...
  function invalidateChapterMemoryCache(cachePath) {
  ...
  async function writeChapterCache(cachePath, chapters) {
    const temporaryPath = path.join(
      path.dirname(cachePath),
      `.${path.basename(cachePath)}.${process.pid}.${Date.now()}.${Math.random().toString(36).slice(2)}.tmp`
    );
    try {
      await fileSystem.writeFile(
        temporaryPath,
        JSON.stringify({ _cacheVersion: chapterCacheVersion, chapters }),
        { flag: 'wx', mode: 0o600 }
      );
  ...
  function attachSourceDocument(chapters, sourceDocument) {
    if (!Array.isArray(chapters) || !sourceDocument) return chapters;
  ...
  function normalizeExtractedChapters(chapters, sourceDocument = chapters?.sourceDocument, sourceFormat = '', work = null) {
    const mutations = chapters?.mutationActivations || chapters?.extractionResult?.mutations || [];
    const normalized = normalizeSequence(splitChapters(chapters), { sourceFormat, work });
    attachMutationActivations(normalized, mutations);
    return attachSourceDocument(normalized, sourceDocument);
  }

  async function extractChapters(bookPath) {
    if (isXBookPath(bookPath)) {
      const xbook = await readXBookArtifact(bookPath);
      return normalizeExtractedChapters(
        xbook.chapters,
        xbook.sourceDocument,
        xbook.sourceFormat,
        xbook.metadata
      );
    }
    ...
    const extraction = (async () => {
      ...
      chapters = normalizeExtractedChapters(chapters, undefined, format);
  ...
  async function extractResult(bookPath) {
    const chapters = await extractChapters(bookPath);
    if (chapters?.extractionResult) return chapters.extractionResult;
    if (isXBookPath(bookPath)) {
      const xbook = await readXBookArtifact(bookPath);
      return fromLegacyChapters(chapters, {
        sourceFormat: xbook.sourceFormat || 'xbook',
        sourceDocument: xbook.sourceDocument,
        processingVersion: Number.isInteger(xbook.processingVersion) ? xbook.processingVersion : 0
      });
    }
    return fromLegacyChapters(chapters, {
      sourceFormat: getFormatFromName(bookPath),
      sourceDocument: chapters?.sourceDocument
    });
  }
  ...
  async function getChaptersCached(bookPath) {
    if (isXBookPath(bookPath)) {
      ...
      if (!repaired) {
        ...
        })), { sourceFormat: xbook.sourceFormat, work: xbook.metadata });
        attachSourceDocument(repaired, xbook.sourceDocument);
    ...
    try {
      ...
      if (cacheIdentity && cacheIdentity.mtimeMs > bookIdentity.mtimeMs) {
        ...
        const readPromise = (async () => {
          ...
          // v30 -> v31 only corrects standalone license metadata. Preserve
          // cached narration/indices so new source-layout rules cannot recut
          // existing playback. Fresh imports use the current extractor.
          if (chapterCacheVersion === 31 && data._cacheVersion === 30 && Array.isArray(data.chapters)) {
            data.chapters = data.chapters.map(chapter => {
              const normalized = normalizeType(chapter);
              return normalized?.type === 'copyright' && chapter?.type !== 'copyright'
                ? { ...chapter, type: 'copyright' } : chapter;
            });
            await writeChapterCache(cachePath, data.chapters);
            data._cacheVersion = 31;
          }
    ...
    if (chapterInflightGenerations.has(cachePath)) {
      return chapterInflightGenerations.get(cachePath);
    }

    const generation = (async () => {
      const chapters = await extractChapters(bookPath);
      try {
        await writeChapterCache(cachePath, chapters);
        invalidateIdentity(cachePath);
        const [bookIdentity, cacheIdentity] = await Promise.all([
          getIdentity(bookPath),
    ...
    chapterInflightGenerations.set(cachePath, generation);
    try {
      return await generation;
    } finally {
      chapterInflightGenerations.delete(cachePath);
    }
  }
  ...
  async function analyzeEpubChapterContent(epubPath, flow, epub) {
    ...
    for (let index = 0; index < chaptersToCheck; index++) {
      try {
        ...
        if (text.length >= SUBSTANTIAL_SECTION_CHARS) {
          chaptersWithContent++;
          consecutiveEmpty = 0;
        } else {
          emptyChapters++;
    ...
    const lengthAssessment = assessReadableContentLength({
      ...
      substantialSections: chaptersWithContent
    });
    if (!lengthAssessment.valid) {
      result.isAudiobookSuitable = false;
      result.error = `${lengthAssessment.error}. This file appears corrupted or is not a complete book.`;
      return result;
    }
    if (lengthAssessment.warning) result.warnings.push(lengthAssessment.warning);
  ...
  return {
    getFormatFromName,
    getChapterCachePath,
    getRebuildArtifactPath,
    canRebuildChapters,
    planChapterRebuild,
    invalidateChapterMemoryCache,
    isXBookPath,
    extractResult,
    extractChapters,
    getChaptersCached,
