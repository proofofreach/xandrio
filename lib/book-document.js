...
function createBookDocument(options = {}) {
  ...
  function normalizeExtractedChapters(chapters, sourceDocument = chapters?.sourceDocument, sourceFormat = '') {
    const mutations = chapters?.mutationActivations || chapters?.extractionResult?.mutations || [];
    const normalized = normalizeSequence(splitChapters(chapters), { sourceFormat });
  ...
  async function extractChapters(bookPath) {
    if (isXBookPath(bookPath)) {
      ...
      return normalizeExtractedChapters(xbook.chapters, xbook.sourceDocument, xbook.sourceFormat);
    }
  ...
  async function getChaptersCached(bookPath) {
    if (isXBookPath(bookPath)) {
      ...
      if (!repaired) {
        ...
        })), { sourceFormat: xbook.sourceFormat });
