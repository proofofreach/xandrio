...
function createBookDocument(options = {}) {
  ...
  function normalizeExtractedChapters(chapters, sourceDocument = chapters?.sourceDocument) {
    const mutations = chapters?.mutationActivations || chapters?.extractionResult?.mutations || [];
    const normalized = normalizeSequence(splitChapters(chapters));
  ...
  async function extractChapters(bookPath) {
    if (isXBookPath(bookPath)) {
      ...
      return normalizeExtractedChapters(xbook.chapters, xbook.sourceDocument);
    }
    ...
    const extraction = (async () => {
      ...
      chapters = normalizeExtractedChapters(chapters);
