...
async function searchLibgen(query, opts = {}) {
  ...
  // Test seam: unit tests inject a fetcher so mirror rotation is exercised
  ...
  for (const baseUrl of mirrors) {
    let html;
    try {
      html = await fetch(baseUrl);
    } catch (error) {
      lastError = error;
      continue;
    }
    const results = parseLibgenResults(html, { baseUrl });
    if (results.length > 0) return results.slice(0, limit);
    // A 200 with no result rows is a real empty answer from a working mirror.
    // Remember it, but keep trying other mirrors in case this fork just indexes
    // the title differently.
    sawCleanEmpty = true;
  }

  if (sawCleanEmpty) return [];
  throw lastError || new Error('LibGen search is unavailable');
}
