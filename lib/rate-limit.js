...
function defaultGroups(max) {
  ...
  // Native HLS polls its EVENT playlist and fetches several finite segments
  // concurrently. Sharing the ordinary TTS bucket (60/min by default) can
  // throttle healthy playback and present as an unexplained lock-screen pause.
  // Session creation has its own much tighter per-account limiter; this
  // transport bucket is a high ceiling that still bounds request floods.
  const playbackMediaMax = Math.max(600, max * 10);
  return [
    ...
    {
      name: 'playback-media',
      max: playbackMediaMax,
      match: path => /^\/api\/(?:audio-hls\/|audio-hls-segment\/|audio-timeline\/)/.test(path)
    },
    {
      name: 'tts',
      max,
      match: path => /^\/api\/(?:audio(?:-ios|-chunked|-continuous)?\/|chunks\/(?:[^/]+\/\d+\/(?:\d+|prepare|retry|prepare-chapter-audio))$|premium-prep\/.*\/start$)/.test(path)
    },
