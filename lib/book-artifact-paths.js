...
function managedArtifactPattern(bookId) {
  ...
  return new RegExp(
    `^${id}(?:` +
      ...
      // stitched narration provenance marker and its atomic temporary
      `|${VARIANT_SEGMENT}?_ch\\d+\\.${CHAPTER_AUDIO_EXT}\\.narration-artifact\\.json(?:\\.\\d+\\.[a-f0-9]+\\.tmp)?` +
      // immutable offline package and committed integrity pointer
      `|_offline_[a-f0-9]{16}_ch\\d+_sha256-${SHA256_HEX}\\.mp3` +
      `|_offline_[a-f0-9]{16}_ch\\d+\\.mp3\\.offline-integrity\\.json` +
      `|_offline-ready_${SHA256_HEX}\\.json(?:\\.\\d+\\.[a-f0-9]+\\.\\d+\\.tmp)?` +
      ...
      `|${VARIANT_SEGMENT}?_ch\\d+_chunk\\d+\\.${CHUNK_AUDIO_EXT}\\.narration-artifact\\.json\\.\\d+\\.[a-f0-9]+\\.tmp` +
      ...
      // unique offline encoder and publication temporary files
      `|_offline_[a-f0-9]{16}_ch\\d+\\.mp3\\.part` +
      `|_offline_[a-f0-9]{16}_ch\\d+\\.mp3\\.\\d+\\.[a-f0-9]+\\.part\\.mp3` +
      `|_offline_[a-f0-9]{16}_ch\\d+\\.mp3\\.offline-integrity\\.json\\.\\d+\\.[a-f0-9]+\\.tmp` +
      `|_offline_[a-f0-9]{16}_ch\\d+_sha256-${SHA256_HEX}\\.mp3\\.\\d+\\.[a-f0-9]+\\.tmp` +
      // per-book cross-variant narration artifact directory
      `|_narration_artifacts_v1` +
    `)$`
  );
}
