...
// records the before/after chapter counts, so an entry stops matching once the
// real counts differ from the declared ones. That check is exact at count
// granularity only: a later, different re-cut landing on the same count would
// still be accepted. Structure keys would settle it, but they are
// content-derived and this report publishes no content hashes.
...
function structureChangeIsAccepted(entry, before, after) {
  ...
  const from = Number(entry.fromChapterCount);
  const to = Number(entry.toChapterCount);
  if (from !== Number(before?.chapterCount) || to !== Number(after?.chapterCount)) return false;
  // A re-cut that leaves the chapter count alone is the one a reader cannot
  // see coming and a diff cannot show: the boundaries moved but the only
  // privacy-safe observable did not. Chapter counts cannot describe it, so the
  // declaration has to say plainly that this is what it is; otherwise an
  // unchanged pair of counts would read as a harmless no-op.
  if (from === to) return entry.structureChangedWithoutChapterCount === true;
  return true;
}

// A case that must conserve narration but exposes neither a structure key nor
// a comparable chapter count cannot be checked at all. Silence there would read
// as "structure conserved", so it counts as an unverifiable case instead.
function hasComparableStructureIdentity(before, after) {
  if (before?.structureKey !== undefined || after?.structureKey !== undefined) return true;
  return Number.isInteger(before?.chapterCount) && Number.isInteger(after?.chapterCount);
}
...
function compareImportBenchmark({ baseline, candidate, acceptedStructureChanges: accepted } = {}) {
  ...
  for (const id of ids) {
    ...
    if (
      (after.mustConserveNarration ?? before.mustConserveNarration) &&
      !hasComparableStructureIdentity(before, after)
    ) {
      summary.uncheckableStructureCases++;
    }
  ...
  const gates = [
    ...
    gate(
      'chapter-structure-is-checkable',
      summary.uncheckableStructureCases === 0,
      summary.uncheckableStructureCases,
      0
    ),
