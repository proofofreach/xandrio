const { GENERATION_PRIORITY } = require('./audio-generation-intent');

/**
 * Priority admission control for scarce generation resources.
 *
 * Callers name a resource (for example `gpu`) and submit work. The scheduler
 * owns capacity and priority ordering; engine and book workflows do not need
 * to know which other producer currently contends for that resource.
 */
class GenerationScheduler {
  constructor({
    capacities = { gpu: 1 },
    backgroundAgingMs = 30000,
    lookaheadBurst = 4,
    downloadMaxWaitMs = 30000,
    now = Date.now
  } = {}) {
    this.capacities = new Map(Object.entries(capacities));
    this.resources = new Map();
    this.sequence = 0;
    this.backgroundAgingMs = Math.max(1, Number(backgroundAgingMs) || 30000);
    this.lookaheadBurst = Math.max(1, Number(lookaheadBurst) || 4);
    this.downloadMaxWaitMs = Math.max(1, Number(downloadMaxWaitMs) || 30000);
    this.now = now;
  }

  run({ resource = 'gpu', priority = 'background' } = {}, work) {
    if (typeof work !== 'function') return Promise.reject(new TypeError('work must be a function'));
    let item;
    const promise = new Promise((resolve, reject) => {
      const state = this._state(resource);
      item = {
        resource,
        priority,
        sequence: this.sequence++,
        enqueuedAt: this.now(),
        work,
        resolve,
        reject,
        started: false,
        settled: false,
        controller: new AbortController()
      };
      state.pending.push(item);
      this._drain(resource, state);
    });
    promise.cancel = () => this._cancel(item);
    promise.promote = priority => this._promote(item, priority);
    return promise;
  }

  hasForegroundWork(resource = 'gpu') {
    const state = this.resources.get(resource);
    if (!state) return false;
    return state.active.some(item => this._isForeground(item.priority)) ||
      state.pending.some(item => this._isForeground(item.priority));
  }

  /**
   * Producer-side yield point. This does not reserve capacity: the actual
   * generation job must still enter through run().
   */
  async waitForBackgroundTurn(resource = 'gpu') {
    while (this.hasForegroundWork(resource)) {
      await new Promise(resolve => this._state(resource).waiters.push(resolve));
    }
  }

  getStatus(resource = 'gpu') {
    const state = this._state(resource);
    return { active: state.active.length, queued: state.pending.length };
  }

  _state(resource) {
    if (!this.resources.has(resource)) {
      this.resources.set(resource, {
        active: [],
        pending: [],
        waiters: [],
        consecutiveLookahead: 0
      });
    }
    return this.resources.get(resource);
  }

  _capacity(resource) {
    const value = Number(this.capacities.get(resource));
    return Number.isFinite(value) && value > 0 ? Math.floor(value) : 1;
  }

  _weight(priority) {
    if (priority === GENERATION_PRIORITY.IMMEDIATE) return 0;
    if (priority === GENERATION_PRIORITY.NEXT) return 1;
    if (priority === GENERATION_PRIORITY.LOOKAHEAD) return 2;
    if (priority === GENERATION_PRIORITY.DOWNLOAD) return 3;
    return 4;
  }

  _effectiveWeight(item) {
    // Live playback must never queue behind speculative preparation. Aging a
    // background render into the foreground class made a large old backlog win
    // FIFO ties against newly requested audio, producing full-generation stalls
    // on single-capacity GPU voices. Background jobs still make progress
    // whenever no interactive or explicit download work is pending.
    return this._weight(item.priority);
  }

  _cancel(item) {
    if (!item || item.settled || item.controller.signal.aborted) return false;
    const state = this.resources.get(item.resource);
    if (item.started) {
      // Admitted work retains its capacity slot until it cooperatively settles;
      // releasing early would exceed the resource capacity while it still runs.
      item.controller.abort();
      return true;
    }
    const index = state?.pending.indexOf(item) ?? -1;
    if (index < 0) return false;
    state.pending.splice(index, 1);
    item.controller.abort();
    item.settled = true;
    const error = new Error('Generation admission cancelled');
    error.name = 'AbortError';
    item.reject(error);
    this._notify(state);
    return true;
  }

  _promote(item, priority) {
    if (!item || item.started || item.settled || item.controller.signal.aborted) return false;
    if (this._weight(priority) >= this._weight(item.priority)) return false;
    item.priority = priority;
    const state = this.resources.get(item.resource);
    if (state) this._drain(item.resource, state);
    return true;
  }

  _isForeground(priority) {
    return priority === 'immediate' ||
      priority === 'next' ||
      priority === 'lookahead' ||
      priority === 'download';
  }

  _takeNext(state) {
    const bySequence = (left, right) => left.sequence - right.sequence;
    const firstOf = priority => state.pending
      .filter(item => item.priority === priority)
      .sort(bySequence)[0] || null;
    const strict = firstOf('immediate') || firstOf('next');
    let selected = strict;

    if (!selected) {
      const lookahead = firstOf('lookahead');
      const download = firstOf('download');
      if (lookahead && download) {
        const downloadWait = this.now() - download.enqueuedAt;
        selected = state.consecutiveLookahead >= this.lookaheadBurst ||
          downloadWait >= this.downloadMaxWaitMs
          ? download
          : lookahead;
      } else {
        selected = lookahead || download;
      }
    }
    if (!selected) {
      selected = state.pending.slice().sort((left, right) =>
        this._effectiveWeight(left) - this._effectiveWeight(right) ||
        left.sequence - right.sequence
      )[0] || null;
    }
    if (!selected) return null;

    state.pending.splice(state.pending.indexOf(selected), 1);
    if (selected.priority === 'lookahead') state.consecutiveLookahead += 1;
    else if (selected.priority === 'download') state.consecutiveLookahead = 0;
    return selected;
  }

  _notify(state) {
    const waiters = state.waiters.splice(0);
    for (const resolve of waiters) resolve();
  }

  _drain(resource, state) {
    while (state.active.length < this._capacity(resource) && state.pending.length) {
      const item = this._takeNext(state);
      if (!item) break;
      item.started = true;
      state.active.push(item);
      Promise.resolve()
        .then(() => item.work({
          signal: item.controller.signal,
          resource: item.resource,
          priority: item.priority
        }))
        .then(value => {
          item.settled = true;
          state.active.splice(state.active.indexOf(item), 1);
          this._notify(state);
          this._drain(resource, state);
          item.resolve(value);
        }, error => {
          item.settled = true;
          state.active.splice(state.active.indexOf(item), 1);
          this._notify(state);
          this._drain(resource, state);
          item.reject(error);
        });
    }
    this._notify(state);
  }
}

module.exports = GenerationScheduler;
