...
function isGarbageTitle(title) {
  ...
  if (/\b[A-Z][a-z]{2,}[A-Z][a-z]{2,}\b/.test(trimmed)) return true;
  return false;
}
...
function isGarbageAuthor(author) {
  ...
  if (/^(?:1[5-9]\d{2}|20\d{2})(?:\s*\([^)]*\))?$/.test(lower)) return true;
  if (/^(?:z-library|z-lib)(?:\s+book)?$/.test(lower)) return true;
  return false;
}
...
function resolveMetadataSeed(metadata, fallbackTitle, fallbackAuthor, filename, storageId) {
  if (filename && storageId) {
    const name = path.basename(filename);
    const stem = name.slice(0, name.length - path.extname(name).length);
    if (stem === storageId) filename = undefined;
    else if (stem.startsWith(`${storageId} `)) filename = name.slice(storageId.length).trim();
  }
  ...
  const filenameAuthor = isGarbageAuthor(filenameMetadata.author) ? undefined : filenameMetadata.author;
