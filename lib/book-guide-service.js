...
function applyGuideRepairs(guide, rawRepairs, expectedPaths) {
  ...
  if (repairs.size !== expected.size || [...expected].some(path => !repairs.get(path)) ||
      [...repairs].some(([path]) => !expected.has(path))) {
    throw guideError('Guide repair response did not match the rejected fields', 'BOOK_GUIDE_GENERATION_INVALID', 502);
  }
    ...
    for (let attempt = Math.max(0, Number(job.attempt) || 0); attempt < MAX_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        if (work.composition?.claimSetHash === claimSetHash && work.composition.guide && pendingRepairPaths.length > 0) {
          ...
          guide = applyGuideRepairs(work.composition.guide, rawRepair?.repairs, pendingRepairPaths);
