...
function mapPrompt({ chapter, segment, segmentIndex, segmentCount, priorFailure }) {
  return [
    ...
    'Each statement must preserve scope and qualifications. Evidence must be copied exactly from this segment.',
...
function publicMessage(status, job) {
  ...
  if (status === 'error') return `Guide generation failed (${job?.errorCode || 'BOOK_GUIDE_GENERATION_FAILED'}).`;
...
function certificationProvenance(config) {
  const identity = model => model?.name && model?.digest ? `${model.name}@${model.digest}` : '';
  return {
    generatorModel: identity(config?.generator),
    verifierModel: identity(config?.verifier),
...
function createBookGuideService({
  ...
  async function getConfig() {
    ...
    const configured = Boolean(config.enabled && config.baseUrl && config.generator?.name &&
      config.generator?.digest && config.verifier?.name && config.verifier?.digest);
    ...
    return {
      ...config,
      ready: configured && certification.certified,
      configured,
  ...
  async function configure(input = {}) {
    ...
    if (!enabled) {
      await abortAll();
      return store.saveConfig({ enabled: false, configuredAt: nowIso(now) });
    }
  ...
  async function runModel({ config, model, prompt, purpose, signal }) {
    ...
    const admission = scheduler.run({ resource: 'gpu', priority: 'background' }, work);
  ...
  async function updateProgress(job, patch) {
    ...
    await journal.update(job.id, patchWithTime(patch));
  }
  ...
  async function generateAttempt({ job, snapshot, config, signal, priorFailure }) {
    const claims = [];
    const skipped = [];
    const workItems = [];
    for (const chapter of snapshot.chapters) {
      ...
      segments.forEach((segment, segmentIndex) => workItems.push({ chapter, segment, segmentIndex, segmentCount: segments.length }));
    }
    await updateProgress(job, { phase: 'extracting', current: 0, total: workItems.length });
    for (let index = 0; index < workItems.length; index++) {
      ...
      for (const [claimIndex, candidate] of array(raw?.claims).entries()) {
        ...
        if (!anchor) {
          throw guideError('Generated evidence did not resolve exactly', 'BOOK_GUIDE_EVIDENCE_UNRESOLVED', 502);
        }
        claims.push({
      ...
      await updateProgress(job, { phase: 'extracting', current: index + 1, total: workItems.length });
    }
    if (claims.length < 5) throw guideError('Too few grounded claims', 'BOOK_GUIDE_GENERATION_INVALID', 502);

    await updateProgress(job, { phase: 'composing', current: 0, total: 1 });
    const rawGuide = await runModel({
      config,
      model: config.generator,
      purpose: 'composition',
      signal,
    ...
    const guide = normalizeGuideOutput(rawGuide, { snapshot, claims, skipped });
    ...
    await updateProgress(job, { phase: 'verifying', current: 0, total: Math.ceil(verificationItems.length / 24) });
    ...
    for (let index = 0; index < batches.length; index++) {
      const raw = await runModel({
        ...
        signal,
        prompt: verificationPrompt(batches[index])
      });
      for (const verdict of array(raw?.verdicts)) {
        verdicts.set(String(verdict?.claimId || ''), verdict?.supported === true);
      }
      await updateProgress(job, { phase: 'verifying', current: index + 1, total: batches.length });
    }
    const unsupported = verificationItems.filter(item => verdicts.get(item.id) !== true);
    if (unsupported.length > 0) {
      throw guideError(`${unsupported.length} claim(s) failed semantic verification`, 'BOOK_GUIDE_GROUNDING_FAILED', 502);
    }
  ...
  async function execute(job, controller) {
    ...
    for (let attempt = Math.max(0, Number(job.attempt) || 0); attempt < MAX_ATTEMPTS; attempt++) {
      job.attempt = attempt;
      await updateProgress(job, { status: 'running', phase: 'starting', attempt, current: 0, total: 0 });
      try {
        artifact = await generateAttempt({ job, snapshot, config, signal: controller.signal, priorFailure });
        break;
      } catch (error) {
        if (controller.signal.aborted || error.code === 'BOOK_GUIDE_CANCELLED') throw error;
  ...
  async function startImpl(bookId, { nonfictionConfirmed = false } = {}) {
    const config = await getConfig();
    ...
    if (nonfictionConfirmed !== true) {
      throw guideError('Confirm that this book is nonfiction', 'BOOK_GUIDE_NONFICTION_CONFIRMATION_REQUIRED', 400);
    }
    ...
    const job = await journal.put({
      ...
      nonfictionConfirmedAt: timestamp,
      createdAt: timestamp,
      updatedAt: timestamp
    });
    pending.push(job);
  ...
  async function get(bookId) {
    ...
    const invalidAnchorIds = stale && source
      ...
      : [];
    const status = publicJobStatus(job, { artifact, stale, configReady: config.ready });
    return {
      feature: { enabled: config.enabled, ready: config.ready, localOnly: true, scope: 'english-nonfiction' },
      featureEnabled: config.enabled,
      canGenerate: config.ready,
      canManage: true,
      status,
      ...
      generation: {
        ...
        estimatedDuration: 'Varies by book length and local hardware',
        estimatedCost: 'No external API charge; uses local compute'
      },
      stale,
      invalidAnchorIds,
  ...
  async function restore() {
    ...
    for (const job of jobs) {
      ...
      const nextAttempt = job.status === 'running' ? Math.min(MAX_ATTEMPTS, job.attempt + 1) : job.attempt;
