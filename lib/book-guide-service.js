...
function createBookGuideService({
  ...
  async function configure(input = {}) {
    ...
    if (!acknowledgementAt) {
      throw guideError(
        'Acknowledge external book processing before enabling PPQ.ai',
  ...
  async function get(bookId) {
    ...
    const status = publicJobStatus(job, { artifact, stale, configReady: titleReady });
    return {
      feature: {
        enabled: config.enabled && nonfictionTagged,
        ready: titleReady,
        localOnly: false,
        externalProcessing: true,
