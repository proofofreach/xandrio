...
function createBookGuideService({
  ...
  async function startImpl(bookId, {
    ...
    if (externalProcessingConfirmed !== true) {
      throw guideError('Confirm external processing', 'BOOK_GUIDE_EXTERNAL_PROCESSING_CONFIRMATION_REQUIRED', 400);
    }
    ...
    const job = await journal.put({
      ...
      externalProcessingConfirmedAt: timestamp,
      createdAt: timestamp,
      updatedAt: timestamp
    });
    pending.push(job);
