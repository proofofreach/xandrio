...
function checkpointIdentity({ snapshot, config }) {
  return {
    ...
    verificationPolicy: config.verificationPolicy || 'configured-verifier-v1'
  };
}
...
function checkpointMatches(work, expected, bookId) {
  return Boolean(work && work.version === WORK_CHECKPOINT_VERSION && work.bookId === bookId &&
    Object.entries(expected).every(([key, value]) => key === 'recipeHash'
      ...
      : key === 'verificationPolicy' ? (work[key] || 'configured-verifier-v1') === value : work[key] === value));
}
...
function normalizeGuideOutput(raw, { snapshot, claims, skipped }) {
  ...
  for (const item of array(raw?.keyPassages).slice(0, 8)) {
    ...
    keyPassages.push({ text: boundedQuoteWords(claim.evidence), anchorId: claim.anchor.id });
  }
...
function applyGuideRepairs(guide, rawRepairs, expectedPaths) {
  ...
  return {
    guide: output,
    repairedPaths,
    missingPaths: [...expected].filter(path => !repairedPaths.includes(path))
  };
}

const SAFE_SELF_EXPLANATION_PROMPTS = Object.freeze([
  'Explain one central idea in your own words, including its limits.',
  'Compare two central ideas and explain how they support the main argument.',
  'Describe how you could test or apply one idea without overstating it.',
  'Identify one important qualification and explain why it matters.'
]);

function replaceUnanchoredQuoteViolation(guide, error) {
  const match = String(error?.guidePath || '').match(/^review\.selfExplanationPrompts\[(\d+)\]$/);
  if (!match) return false;
  const index = Number(match[1]);
  if (!Array.isArray(guide?.review?.selfExplanationPrompts) || index >= guide.review.selfExplanationPrompts.length) {
    return false;
  }
  guide.review.selfExplanationPrompts[index] = SAFE_SELF_EXPLANATION_PROMPTS[index % SAFE_SELF_EXPLANATION_PROMPTS.length];
  return true;
}
...
function validateCertificationReport(report, expected) {
  ...
  if ((report.provenance?.verificationPolicy || 'configured-verifier-v1') !== (expected.verificationPolicy || 'configured-verifier-v1')) {
...
function createBookGuideService({
  loadBook,
  getChapters,
  getSourceVersion = async book => book.chapterStructureKey || book.sourceHash || '',
  store,
  journal,
  provider,
  scheduler = null,
  ...
  sourceSnapshotTtlMs = 30_000,
  maxSourceSnapshots = 100,
  ...
  const boundedSourceSnapshotTtlMs = Math.max(1000, Number(sourceSnapshotTtlMs) || 30_000);
  const boundedMaxSourceSnapshots = Math.max(1, Number(maxSourceSnapshots) || 100);

  function sourceSnapshotKey(book, sourceVersion) {
    return JSON.stringify([
      book.path,
      book.chapterStructureKey,
      book.sourceHash,
      book.processingVersion,
      book.title,
      book.author,
      book.language,
      book.studyGuideCategory,
      sourceVersion,
      book.chapterCount
    ]);
  }

  function cacheSourceSnapshot(bookId, key, snapshot) {
    sourceSnapshots.delete(bookId);
    sourceSnapshots.set(bookId, {
      key,
      snapshot,
      expiresAt: Date.now() + boundedSourceSnapshotTtlMs
    });
    while (sourceSnapshots.size > boundedMaxSourceSnapshots) {
      sourceSnapshots.delete(sourceSnapshots.keys().next().value);
    }
  }
  ...
  async function sourceForBook(bookId) {
    return withBookStateLock(bookId, async () => {
      ...
      const key = sourceSnapshotKey(book, await getSourceVersion(book));
      const cached = sourceSnapshots.get(bookId);
      if (cached?.key === key && cached.expiresAt > Date.now()) {
        return { book, snapshot: cached.snapshot };
      }
      const chapters = await getChapters(book.path, bookId, book);
      const snapshot = createBookGuideSourceSnapshot({ bookId, book, chapters });
      cacheSourceSnapshot(bookId, key, snapshot);
      return { book, snapshot };
    });
  }

  async function getConfig() {
    const config = { ...(await store.loadConfig()), verificationPolicy: semanticVerifier?.active ? [semanticVerifier.policyId, provider.verificationPolicy].filter(Boolean).join('+') : provider.verificationPolicy };
  ...
  async function providerLoginStatus() {
    ...
    return { available: true, connected, state: connected ? 'connected' : 'disconnected' };
  }
  ...
  async function generateAttempt({ job, snapshot, config, signal }) {
    ...
    for (let attempt = Math.max(0, Number(job.attempt) || 0); attempt < MAX_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        // Quote controls are deterministic. Run them before semantic verification
        // so an over-literal field can be repaired without repeating verifier work.
        // Self-explanation prompts have no anchors, so replace only a violating
        // prompt with a safe study instruction and leave the rest untouched.
        for (let quotePass = 0; quotePass < SAFE_SELF_EXPLANATION_PROMPTS.length; quotePass++) {
          try {
            validatePersistedQuoteControls({ guide }, snapshot);
            break;
          } catch (error) {
            if (error.code !== 'BOOK_GUIDE_QUOTE_LIMIT' || !replaceUnanchoredQuoteViolation(guide, error)) {
              throw error;
            }
            work.composition = { claimSetHash, guide };
            await writer.persist();
        ...
        const verificationMetrics = { jevAccepted: 0, jevRejected: 0, escalated: 0, fullFallback: 0, elapsedMs: 0, reusedSupportedItems: verificationItems.filter(item => supported[verificationItemKey(item)] === true).length };
        const batches = [];
        ...
        try {
          await eachConcurrent(batches.filter(batch => !batchComplete(batch)), semanticVerifier?.active ? 1 : modelLimit, async batch => {
            const missing = batch.filter(item => supported[verificationItemKey(item)] !== true);
            const fallback = items => runModelWithRetry({
              config,
              model: config.verifier,
              purpose: 'verification',
              signal,
              prompt: verificationPrompt(items)
            });
            const raw = semanticVerifier?.active
              ? await semanticVerifier.verify(missing, { fallback, signal })
              : await fallback(missing);
            for (const key of ['jevAccepted', 'jevRejected', 'escalated', 'fullFallback', 'elapsedMs']) {
              verificationMetrics[key] += Number(raw?.verificationMetrics?.[key] || 0);
            }
        ...
        const artifact = {
          ...
          anchors,
          verification: {
            policy: config.verificationPolicy || 'configured-verifier-v1',
            ...(semanticVerifier?.active ? { lastAttemptMetrics: verificationMetrics } : {}),
        ...
        const quoteRepairPath = String(error.guidePath || '').replace(
          /^(orientation\.(?:thesis|problem|bottomLine|takeaways\[\d+\]))\.text$/,
          '$1'
        );
        const unsupportedGuidePaths = [...new Set([
          ...array(error.unsupported)
            .map(item => String(item?.path || ''))
            .filter(path => path && !path.startsWith('extractedClaims.')),
          quoteRepairPath
