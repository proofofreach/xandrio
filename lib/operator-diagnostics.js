...
const QUEUE_PRIORITIES = Object.freeze(Object.values(GENERATION_PRIORITY));
const QUEUE_ORIGINS = Object.freeze([
  ...Object.values(GENERATION_ORIGIN)
]);
...
function redactedQueue(value) {
  const countFields = (source, allowed) => Object.fromEntries(
    allowed
      .filter(name => safeCount(source?.[name]) > 0)
      .map(name => [name, safeCount(source[name])])
  );
  const result = {
    active: safeCount(value?.active),
    queued: safeCount(value?.queued),
    completed: safeCount(value?.completed),
    byPriority: countFields(value?.byPriority, QUEUE_PRIORITIES),
    byOrigin: countFields(value?.byOrigin, QUEUE_ORIGINS),
    oldestQueuedAgeMs: safeCount(value?.oldestQueuedAgeMs)
  };
  if (value?.artifacts && typeof value.artifacts === 'object') {
    result.artifacts = {
      hits: safeCount(value.artifacts.hits),
      misses: safeCount(value.artifacts.misses),
      published: safeCount(value.artifacts.published),
      bytesAvoided: safeCount(value.artifacts.bytesAvoided)
    };
  }
  return result;
}
