...
// A single search page resolves covers for every result, so one rate-limited
// provider used to produce one failed request per result (hundreds of 429s and
// a second of latency each). Remember the refusal per origin instead and skip
// that provider until its window has passed, so the chain falls through to the
// providers that are still answering.
const rateLimitedOrigins = new Map();
const RATE_LIMIT_COOLDOWN_MS = 5 * 60 * 1000;
// A provider that answers 429 with an implausibly short window would otherwise
// disable the pause entirely and restore the per-result request storm.
const MIN_RATE_LIMIT_COOLDOWN_MS = 1000;
...
function rateLimitRemainingMs(origin) {
  if (!origin) return 0;
  const until = rateLimitedOrigins.get(origin);
  if (!until) return 0;
  const remaining = until - Date.now();
  if (remaining <= 0) {
    rateLimitedOrigins.delete(origin);
    return 0;
  }
  return remaining;
}

function recordRateLimit(origin, retryAfter) {
  if (!origin) return 0;
  const cooldown = Math.min(
    Math.max(parseRetryAfterMs(retryAfter) || RATE_LIMIT_COOLDOWN_MS, MIN_RATE_LIMIT_COOLDOWN_MS),
    MAX_RATE_LIMIT_COOLDOWN_MS
  );
  const until = Date.now() + cooldown;
  const previous = rateLimitedOrigins.get(origin) || 0;
  if (until > previous) rateLimitedOrigins.set(origin, until);
  if (!previous || previous <= Date.now()) {
    console.log(`[cover] ${origin} is rate limited; pausing that source for ${Math.round(cooldown / 1000)}s`);
  }
  return cooldown;
}

function resetRemoteRateLimits() {
  rateLimitedOrigins.clear();
}

function isRemoteOriginRateLimited(url) {
  return rateLimitRemainingMs(originOf(url)) > 0;
}

// A skipped request is an expected consequence of an earlier refusal, not a new
// fault, so it must not be reported as one per remaining search result.
function isRateLimitSkip(error) {
  return Boolean(error?.rateLimitSkipped);
}

async function fetchRemoteBuffer(url, options, maxBytes = MAX_COVER_BYTES, timeoutMs = COVER_TIMEOUT_MS, maxRedirects = 3) {
  const origin = originOf(url);
  const cooling = rateLimitRemainingMs(origin);
  if (cooling > 0) {
    const skipped = new Error(`Remote request skipped: ${origin} is rate limited for another ${Math.round(cooling / 1000)}s`);
    skipped.rateLimitSkipped = true;
    throw skipped;
  }
  ...
  try {
    if (!remote.response.ok) {
      if (remote.response.status === 429) {
        recordRateLimit(origin, remote.response.headers?.get?.('retry-after'));
      }
      throw new Error(`Remote request failed: ${remote.response.status}`);
    }
    if (origin) rateLimitedOrigins.delete(origin);
...
// Write a cover image atomically (unique temp + rename) so an interrupted
// fetch can never leave a truncated file that later reads treat as valid,
// and concurrent fetches for the same cover cannot truncate each other.
async function writeCoverAtomic(outputPath, data) {
  ...
  const partPath = `${outputPath}.${process.pid}.${crypto.randomBytes(4).toString('hex')}.part`;
...
async function fetchCoverFromGoogleBooks(title, author, outputPath, options = {}) {
  try {
    ...
    if (isRemoteOriginRateLimited(searchUrl)) return false;
    ...
    if (!isRateLimitSkip(err)) console.error('[cover] Google Books error:', err.message);
    return false;
  }
}
...
async function fetchCoverFromOpenLibrary(title, author, outputPath, options = {}) {
  try {
    ...
    if (!isRateLimitSkip(err)) console.error('Open Library API error:', err.message);
    return false;
  }
}
...
async function fetchCoverByOpenLibraryWorkKey(workKey, outputPath, options = {}) {
  try {
    ...
    if (!isRateLimitSkip(err)) console.error('[cover] Open Library work cover error:', err.message);
    return false;
  }
}
...
async function fetchCoverFromGutenbergId(gutenbergId, outputPath, options = {}) {
  ...
  for (const coverUrl of candidates) {
    try {
      ...
      if (!isRateLimitSkip(err)) console.error('[cover] Gutenberg cover error:', err.message);
    }
  }
