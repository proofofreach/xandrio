...
function createXBookStore({
  ...
  async function reprocessXBookArtifact(xbookPath, options = {}) {
    ...
    await fs.writeFile(xbookPath, JSON.stringify(updated));
    invalidateXBookArtifactCache(xbookPath);
    const identity = await getFileIdentity(xbookPath);
    rememberXBookArtifact(`${xbookPath}:${identity.mtimeMs}:${identity.size}`, updated);
    return updated;
  }
