...
function createXBookStore({
  ...
  async function writeXBookArtifactAtomically(xbookPath, artifact) {
    const tempPath = `${xbookPath}.${process.pid}.${++artifactTempSequence}.tmp`;
    let tempFile;
    try {
      tempFile = await fs.open(tempPath, 'wx');
      await tempFile.writeFile(JSON.stringify(artifact));
      await tempFile.close();
      tempFile = null;
      await fs.rename(tempPath, xbookPath);
    } catch (error) {
      if (tempFile) await tempFile.close().catch(() => {});
