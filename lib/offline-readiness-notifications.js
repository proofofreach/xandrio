...
function createOfflineReadinessNotifications({
  ...
  const boundedSendTimeoutMs = Math.max(10, Math.min(30_000, Number(sendTimeoutMs) || 5000));
  const boundedConcurrentSends = Math.max(1, Math.min(16, Number(maxConcurrentSends) || 4));

  async function sendWithDeadline(subscription, payload) {
    let timeoutId;
    const deadline = new Promise((_, reject) => {
      timeoutId = setTimeout(() => {
        const error = new Error('Web Push delivery timed out');
        error.code = 'WEB_PUSH_TIMEOUT';
        reject(error);
      }, boundedSendTimeoutMs);
    });
    try {
  ...
  async function notifyOwners(ownerIds, book = {}) {
    ...
    async function worker() {
      while (cursor < subscriptions.length) {
        const { owner, subscription } = subscriptions[cursor++];
        try {
          // The endpoint is a client-supplied URL that the server later fetches
          // from a background worker, so it is re-validated at use time to
          // prevent stored SSRF and DNS rebinding.
          await assertTarget(subscription.endpoint);
          await sendWithDeadline(subscription, payload);
          report.sent += 1;
        } catch (error) {
    ...
    await Promise.all(
      Array.from({ length: Math.min(boundedConcurrentSends, subscriptions.length) }, () => worker())
    );
    return report;
  }
