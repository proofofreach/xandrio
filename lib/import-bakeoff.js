...
function compareImportBakeoff({ baseline, candidate } = {}) {
  ...
  for (const id of ids) {
    ...
    if (hasNewIdentity(baselineCase, candidateCase, 'warningKeys', 'warningCount')) {
    ...
    if (importChanged || narrationChanged || structureChanged || defectsChanged || warningsChanged || errorsChanged) {
  ...
  const gates = [
    ...
    gate('no-added-warnings', summary.warningRegressions === 0, summary.warningRegressions, 0),
