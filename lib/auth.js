// Authentication for a self-hosted instance.
//
// Two modes share this module:
//
// - Shared-token mode (legacy, and the bootstrap state before any account
//   exists): a configured XANDRIO_TOKEN protects every /api route except the
//   small authentication bootstrap surface. Browser clients exchange the
//   shared token once for a signed, HttpOnly session cookie; non-browser
//   clients can keep using Authorization: Bearer <token>.
//
// - Account mode (one or more username/password accounts in accounts.json):
//   browsers log in with credentials and receive an opaque random session
//   token stored server-side (data/sessions.json, hashed), so sessions are
//   revocable on logout and password change. The shared token, when still
//   configured, remains valid as an admin-equivalent Bearer credential for
//   scripts and monitoring. With zero accounts and no token, the documented
//   trusted-LAN mode applies.
//
// The application shell and /health deliberately stay public so
// service-worker installation and uptime checks do not require credentials.

const crypto = require('crypto');
const { posix } = require('path');
const { unauthorized, forbidden, badRequest, notFound, storageError } = require('./http-error');

const SESSION_COOKIE = 'xandrio_session';
const DEFAULT_SESSION_TTL_MS = 30 * 24 * 60 * 60 * 1000;
const MAX_SESSION_TTL_MS = 90 * 24 * 60 * 60 * 1000;
// Sliding sessions: a session used within this window of its issue (or last
// renewal) keeps its expiry; past it, active use re-issues the full TTL. So a
// device in regular use never re-prompts, at the cost of at most one store
// write and Set-Cookie per session per window.
const SESSION_RENEW_MIN_AGE_MS = 24 * 60 * 60 * 1000;
const SESSION_TOKEN_BYTES = 32;
const PUBLIC_API_PATHS = new Set([
  '/api/auth/login',
  '/api/auth/logout',
  '/api/auth/status',
  '/api/integrations/calibre/claim',
  '/api/integrations/calibre/status',
  '/api/integrations/calibre/inventory',
  '/api/integrations/calibre/import'
]);

function timingSafeEqualStrings(a, b) {
  // Hash both sides to fixed length so timingSafeEqual cannot throw on a
  // length mismatch or reveal either value's length.
  const ha = crypto.createHash('sha256').update(String(a)).digest();
  const hb = crypto.createHash('sha256').update(String(b)).digest();
  return crypto.timingSafeEqual(ha, hb);
}

function parseCookies(header) {
  const cookies = {};
  if (!header) return cookies;
  for (const part of header.split(';')) {
    const idx = part.indexOf('=');
    if (idx === -1) continue;
    const name = part.slice(0, idx).trim();
    if (!name) continue;
    // First occurrence wins. A duplicate cookie name is not a normal thing for
    // a browser to send on its own; it is how an attacker who can set a cookie
    // on a sibling host shadows the real session value, since the injected one
    // often sorts later in the header. Browsers place the more specific cookie
    // first, so keeping the first occurrence keeps the genuine one.
    if (Object.prototype.hasOwnProperty.call(cookies, name)) continue;
    try {
      cookies[name] = decodeURIComponent(part.slice(idx + 1).trim());
    } catch {
      cookies[name] = part.slice(idx + 1).trim();
    }
  }
  return cookies;
}

function requestBearerToken(req) {
  const header = req.headers.authorization || '';
  if (/^Bearer\s+/i.test(header)) return header.replace(/^Bearer\s+/i, '').trim();
  return null;
}

function clampTtl(ttlMs) {
  return Math.min(Math.max(Number(ttlMs) || DEFAULT_SESSION_TTL_MS, 1), MAX_SESSION_TTL_MS);
}

// ─── Legacy stateless shared-token sessions ────────────────────────────────

function sessionSignature(payload, token) {
  return crypto.createHmac('sha256', token).update(payload).digest('base64url');
}

function createSession(token, { now = Date.now(), ttlMs = DEFAULT_SESSION_TTL_MS } = {}) {
  const expiresAt = now + clampTtl(ttlMs);
  const payload = Buffer.from(JSON.stringify({ v: 1, exp: expiresAt })).toString('base64url');
  return `${payload}.${sessionSignature(payload, token)}`;
}

function verifySession(session, token, { now = Date.now() } = {}) {
  if (typeof session !== 'string') return false;
  const [payload, signature, ...extra] = session.split('.');
  if (!payload || !signature || extra.length) return false;
  if (!timingSafeEqualStrings(signature, sessionSignature(payload, token))) return false;
  try {
    const parsed = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8'));
    return parsed?.v === 1 && Number.isFinite(parsed.exp) && parsed.exp > now;
  } catch {
    return false;
  }
}

function legacySessionExpiry(session) {
  try {
    const parsed = JSON.parse(Buffer.from(String(session).split('.')[0], 'base64url').toString('utf8'));
    return Number.isFinite(parsed?.exp) ? parsed.exp : null;
  } catch {
    return null;
  }
}

function requestToken(req, token) {
  const bearer = requestBearerToken(req);
  if (bearer) return bearer;
  const session = parseCookies(req.headers.cookie)[SESSION_COOKIE];
  // The configured token stays server-side; this internal helper returns it
  // only after a valid signed session has been verified.
  return token && verifySession(session, token) ? token : null;
}

// ─── Server-side account sessions ──────────────────────────────────────────

function hashSessionToken(token) {
  return crypto.createHash('sha256').update(String(token)).digest('hex');
}

function normalizeSessionsStore(raw) {
  const store = raw && typeof raw === 'object' ? raw : {};
  if (!store.sessions || typeof store.sessions !== 'object') store.sessions = {};
  return store;
}

function validateSessionsStore(raw) {
  if (raw === null || typeof raw !== 'object' || Array.isArray(raw)) {
    return 'top level must be an object';
  }
  if (raw.sessions !== undefined &&
      (raw.sessions === null || typeof raw.sessions !== 'object' || Array.isArray(raw.sessions))) {
    return 'sessions must be an object when present';
  }
  return true;
}

/**
 * Revocable sessions backed by a json-store file. The cookie holds an opaque
 * random token; only its sha256 is persisted, so the store file never
 * contains a usable credential.
 */
function createSessionStore({ filePath, jsonStore, ttlMs = DEFAULT_SESSION_TTL_MS, now = Date.now }) {
  const sessionTtlMs = clampTtl(ttlMs);
  const persistence = typeof jsonStore.createCriticalStore === 'function'
    ? jsonStore.createCriticalStore({
        filePath,
        defaultValue: {},
        validate: validateSessionsStore,
        maxBackups: 5
      })
    : {
        // Preserve the small injected-store contract used by embedders and
        // tests. Production uses the critical-store path above.
        load: () => jsonStore.load(filePath, {}),
        update: mutator => jsonStore.update(filePath, mutator, {})
      };

  function prune(store, nowMs) {
    for (const [key, session] of Object.entries(store.sessions)) {
      if (!session || !Number.isFinite(session.expiresAtMs) || session.expiresAtMs <= nowMs) {
        delete store.sessions[key];
      }
    }
  }

  return {
    ttlMs: sessionTtlMs,

    async create(userId, { deviceId = null, deviceName = null } = {}) {
      const token = crypto.randomBytes(SESSION_TOKEN_BYTES).toString('base64url');
      const nowMs = now();
      const session = {
        userId,
        deviceId: deviceId || null,
        deviceName: typeof deviceName === 'string' && deviceName.trim() ? deviceName.trim().slice(0, 80) : null,
        createdAt: new Date(nowMs).toISOString(),
        expiresAt: new Date(nowMs + sessionTtlMs).toISOString(),
        expiresAtMs: nowMs + sessionTtlMs
      };
      await persistence.update((data) => {
        const store = normalizeSessionsStore(data);
        prune(store, nowMs);
        store.sessions[hashSessionToken(token)] = session;
      });
      return { token, expiresAtMs: session.expiresAtMs };
    },

    async resolve(token) {
      if (typeof token !== 'string' || !token) return null;
      const store = normalizeSessionsStore(await persistence.load());
      const session = store.sessions[hashSessionToken(token)];
      if (!session || !Number.isFinite(session.expiresAtMs) || session.expiresAtMs <= now()) return null;
      return session;
    },

    /**
     * Extends a live session to a full TTL from now (sliding renewal).
     * Returns the new expiresAtMs, or null when the session no longer exists.
     */
    async touch(token) {
      if (typeof token !== 'string' || !token) return null;
      const nowMs = now();
      let renewedExpiresAtMs = null;
      await persistence.update((data) => {
        const store = normalizeSessionsStore(data);
        prune(store, nowMs);
        const session = store.sessions[hashSessionToken(token)];
        if (session) {
          session.expiresAt = new Date(nowMs + sessionTtlMs).toISOString();
          session.expiresAtMs = nowMs + sessionTtlMs;
          renewedExpiresAtMs = session.expiresAtMs;
        }
      });
      return renewedExpiresAtMs;
    },

    async destroy(token) {
      if (typeof token !== 'string' || !token) return;
      await persistence.update((data) => {
        const store = normalizeSessionsStore(data);
        prune(store, now());
        delete store.sessions[hashSessionToken(token)];
      });
    },

    async destroyAllForUser(userId, { exceptToken = null } = {}) {
      const keepKey = exceptToken ? hashSessionToken(exceptToken) : null;
      await persistence.update((data) => {
        const store = normalizeSessionsStore(data);
        prune(store, now());
        for (const [key, session] of Object.entries(store.sessions)) {
          if (session.userId === userId && key !== keepKey) delete store.sessions[key];
        }
      });
    },

    /**
     * Live sessions for one account, newest first. The public `id` is a
     * prefix of the stored token hash: enough to address a session for
     * revocation, useless for authenticating as it.
     */
    async listForUser(userId, { currentToken = null } = {}) {
      const currentKey = currentToken ? hashSessionToken(currentToken) : null;
      const store = normalizeSessionsStore(await persistence.load());
      const nowMs = now();
      return Object.entries(store.sessions)
        .filter(([, session]) => session && session.userId === userId &&
          Number.isFinite(session.expiresAtMs) && session.expiresAtMs > nowMs)
        .map(([key, session]) => ({
          id: sessionPublicId(key),
          deviceId: session.deviceId || null,
          deviceName: session.deviceName || null,
          createdAt: session.createdAt || null,
          expiresAt: session.expiresAt || null,
          current: key === currentKey
        }))
        .sort((a, b) => Number(b.current) - Number(a.current) || String(b.createdAt).localeCompare(String(a.createdAt)));
    },

    /** Revokes one of `userId`'s sessions by public id. Returns true when a session was removed. */
    async destroyById(userId, sessionId) {
      if (typeof sessionId !== 'string' || !/^[a-f0-9]{16}$/.test(sessionId)) return false;
      let removed = false;
      await persistence.update((data) => {
        const store = normalizeSessionsStore(data);
        prune(store, now());
        for (const [key, session] of Object.entries(store.sessions)) {
          if (session.userId === userId && sessionPublicId(key) === sessionId) {
            delete store.sessions[key];
            removed = true;
          }
        }
      });
      return removed;
    }
  };
}

function sessionPublicId(storeKey) {
  return String(storeKey).slice(0, 16);
}

/**
 * Resolves the requesting identity, or null when unauthenticated.
 *
 * Returned users carry `id` (account id) when a real account session is
 * present. Legacy shared-token and trusted-LAN callers get `id: null` plus a
 * `legacy`/`lan` marker — data routes then fall back to their historical
 * identity resolution.
 */
async function resolveRequestUser(req, { token, accounts, sessionStore }) {
  const accountCount = accounts ? await accounts.count() : 0;

  if (accountCount === 0) {
    if (!token) return { id: null, role: 'admin', lan: true };
    const provided = requestToken(req, token);
    if (provided && timingSafeEqualStrings(provided, token)) {
      return { id: null, role: 'admin', legacy: true };
    }
    return null;
  }

  // Accounts exist: the shared token (when configured) stays valid as an
  // admin-equivalent Bearer credential for scripts and monitoring.
  const bearer = requestBearerToken(req);
  if (token && bearer && timingSafeEqualStrings(bearer, token)) {
    return { id: null, role: 'admin', legacy: true };
  }

  const cookieToken = parseCookies(req.headers.cookie)[SESSION_COOKIE];
  if (cookieToken && sessionStore) {
    const session = await sessionStore.resolve(cookieToken);
    if (session) {
      const account = await accounts.findById(session.userId);
      if (account && !account.disabled) {
        return {
          id: account.id,
          username: account.username,
          displayName: account.displayName,
          role: account.role,
          sessionToken: cookieToken,
          sessionExpiresAtMs: session.expiresAtMs
        };
      }
    }
  }
  return null;
}

// The single canonical form for every path-based security decision.
//
// Express matches routes case-insensitively by default, so a raw string
// compare against '/api/' disagrees with the router: GET /API/library was
// classified public here and then still matched app.get('/api/library').
// Percent-encoding, duplicate slashes and dot segments are the same class of
// disagreement, so all of them are collapsed here rather than case alone.
//
// The rate limiter matches on the same lowercase literals and must use this
// too, or the two gates can drift apart again.
function canonicalRequestPath(req) {
  const raw = req.path || req.url?.split('?')[0] || '';
  let decoded = raw;
  try {
    decoded = decodeURIComponent(raw);
  } catch {
    // Malformed percent-encoding: keep the raw value. A path we cannot decode
    // must not be treated as public.
  }
  return posix.normalize(decoded.replace(/\/{2,}/g, '/')).toLowerCase();
}

function isPublicRequest(req) {
  const path = canonicalRequestPath(req);
  if (!path.startsWith('/api/')) return true;
  return PUBLIC_API_PATHS.has(path);
}

// Re-signs a legacy shared-token session cookie with a fresh TTL when the
// current one is more than SESSION_RENEW_MIN_AGE_MS old. Bearer clients hold
// no cookie and are skipped.
function renewLegacySessionIfDue(req, res, token, ttlMs, now) {
  if (!token || typeof res.cookie !== 'function' || requestBearerToken(req)) return;
  const session = parseCookies(req.headers.cookie)[SESSION_COOKIE];
  if (!verifySession(session, token, { now: now() })) return;
  const exp = legacySessionExpiry(session);
  if (exp === null || exp - now() >= clampTtl(ttlMs) - SESSION_RENEW_MIN_AGE_MS) return;
  const options = sessionCookieOptions(req, { ttlMs });
  res.cookie(SESSION_COOKIE, createSession(token, { now: now(), ttlMs: options.maxAge }), options);
}

// Extends an account session in the store and re-issues its cookie with a
// fresh maxAge once the session is past the renewal window.
async function renewAccountSessionIfDue(req, res, user, sessionStore, now) {
  if (!user?.sessionToken || typeof sessionStore?.touch !== 'function') return;
  const ttlMs = sessionStore.ttlMs;
  if (!Number.isFinite(user.sessionExpiresAtMs) || !Number.isFinite(ttlMs)) return;
  if (user.sessionExpiresAtMs - now() >= ttlMs - SESSION_RENEW_MIN_AGE_MS) return;
  const renewed = await sessionStore.touch(user.sessionToken);
  if (!renewed || typeof res.cookie !== 'function') return;
  res.cookie(SESSION_COOKIE, user.sessionToken, sessionCookieOptions(req, { ttlMs }));
}

/**
 * @param {Object} options
 * @param {string|null} options.token Shared secret; falsy enables trusted-LAN
 *   mode while no accounts exist.
 * @param {Object} [options.accounts] Accounts store; enables account mode.
 * @param {Object} [options.sessionStore] Server-side session store.
 * @param {number} [options.sessionTtlMs] TTL granted on sliding renewal.
 * @param {Function} [options.now] Clock override for tests.
 * @returns Express middleware requiring auth for private API routes.
 */
function createAuthMiddleware({ token, accounts = null, sessionStore = null, sessionTtlMs = DEFAULT_SESSION_TTL_MS, now = Date.now }) {
  if (!accounts) {
    // Legacy shared-token middleware.
    return function requireAuth(req, res, next) {
      if (!token || req.method === 'OPTIONS' || isPublicRequest(req)) return next();
      const provided = requestToken(req, token);
      if (provided && timingSafeEqualStrings(provided, token)) {
        renewLegacySessionIfDue(req, res, token, sessionTtlMs, now);
        return next();
      }
      res.setHeader('WWW-Authenticate', 'Bearer realm="Xandrio"');
      return unauthorized(res);
    };
  }

  return async function requireAuth(req, res, next) {
    if (req.method === 'OPTIONS' || isPublicRequest(req)) return next();
    let user;
    try {
      user = await resolveRequestUser(req, { token, accounts, sessionStore });
    } catch (err) {
      return next(err);
    }
    if (user) {
      req.user = user;
      // Sliding renewal must never fail an authenticated request.
      try {
        if (user.sessionToken) {
          await renewAccountSessionIfDue(req, res, user, sessionStore, now);
        } else {
          renewLegacySessionIfDue(req, res, token, sessionTtlMs, now);
        }
      } catch (err) {
        console.warn('Session renewal failed:', err.message);
      }
      return next();
    }
    res.setHeader('WWW-Authenticate', 'Bearer realm="Xandrio"');
    return unauthorized(res);
  };
}

function requireAdmin(req, res, next) {
  if (req.user?.role === 'admin') return next();
  return forbidden(res, 'Admin access required');
}

// `req.secure` is false unless XANDRIO_TRUST_PROXY is set, but the default
// `npm start` puts the server behind a TLS-terminating tailscale proxy, so the
// out-of-the-box HTTPS deployment was issuing session cookies without Secure.
//
// X-Forwarded-Proto is consulted here even when `trust proxy` is off. Doing so
// cannot weaken the cookie: a forged `https` only sets Secure on a plaintext
// connection, which at worst stops the forger's own browser from returning the
// cookie. Turning Secure *off* would require suppressing a header that a real
// TLS proxy always adds, which a remote client cannot do.
function requestIsHttps(req) {
  if (req.secure) return true;
  const forwarded = req.headers?.['x-forwarded-proto'];
  if (typeof forwarded !== 'string') return false;
  return forwarded.split(',')[0].trim().toLowerCase() === 'https';
}

function secureCookiePolicy(raw = process.env.XANDRIO_SECURE_COOKIES) {
  const value = String(raw || 'auto').trim().toLowerCase();
  return value === 'always' || value === 'never' ? value : 'auto';
}

function sessionCookieOptions(req, { ttlMs = DEFAULT_SESSION_TTL_MS, securePolicy } = {}) {
  // Normalized through the same parser so an unrecognized explicit value
  // falls back to 'auto' rather than silently behaving like it.
  const policy = secureCookiePolicy(securePolicy || process.env.XANDRIO_SECURE_COOKIES);
  return {
    httpOnly: true,
    sameSite: 'lax',
    secure: policy === 'always' ? true : policy === 'never' ? false : requestIsHttps(req),
    path: '/',
    maxAge: clampTtl(ttlMs)
  };
}

function createAuthRoutes({ token, sessionTtlMs = DEFAULT_SESSION_TTL_MS, accounts = null, sessionStore = null }) {
  function legacyLogin(req, res) {
    if (!token) return res.status(204).end();
    const provided = typeof req.body?.token === 'string' ? req.body.token.trim() : '';
    if (!provided || !timingSafeEqualStrings(provided, token)) {
      return unauthorized(res);
    }
    const options = sessionCookieOptions(req, { ttlMs: sessionTtlMs });
    res.cookie(SESSION_COOKIE, createSession(token, { ttlMs: options.maxAge }), options);
    return res.status(204).end();
  }

  function clearSessionCookie(res, req) {
    const { maxAge: _maxAge, ...clearOptions } = sessionCookieOptions(req);
    res.clearCookie(SESSION_COOKIE, clearOptions);
  }

  return {
    async login(req, res) {
      try {
        if (!accounts || (await accounts.count()) === 0) {
          // Bootstrap: the shared token remains the browser credential until
          // the first account exists.
          return legacyLogin(req, res);
        }
        const username = typeof req.body?.username === 'string' ? req.body.username : '';
        const password = typeof req.body?.password === 'string' ? req.body.password : '';
        const account = await accounts.verifyLogin(username, password);
        if (!account) {
          return unauthorized(res, 'Invalid username or password');
        }
        const options = sessionCookieOptions(req, { ttlMs: sessionTtlMs });
        const { token: sessionToken } = await sessionStore.create(account.id, {
          deviceId: req.headers['x-xandrio-device-id'] || null,
          deviceName: req.headers['x-xandrio-device-name'] || null
        });
        res.cookie(SESSION_COOKIE, sessionToken, options);
        return res.json({
          success: true,
          user: { id: account.id, username: account.username, displayName: account.displayName, role: account.role }
        });
      } catch (err) {
        return storageError(res, err, 'Login failed', 'Login failed:');
      }
    },

    async logout(req, res) {
      try {
        if (sessionStore) {
          await sessionStore.destroy(parseCookies(req.headers.cookie)[SESSION_COOKIE]);
        }
        clearSessionCookie(res, req);
        return res.status(204).end();
      } catch (err) {
        return storageError(res, err, 'Logout failed', 'Logout failed:');
      }
    },

    async status(req, res) {
      try {
        if (!accounts) {
          return res.json({ authenticationRequired: Boolean(token) });
        }
        const accountCount = await accounts.count();
        const user = await resolveRequestUser(req, { token, accounts, sessionStore });
        return res.json({
          authenticationRequired: accountCount > 0 || Boolean(token),
          accountsConfigured: accountCount > 0,
          authenticated: Boolean(user),
          user: user?.id
            ? { id: user.id, username: user.username, displayName: user.displayName, role: user.role }
            : null
        });
      } catch (err) {
        return storageError(res, err, 'Failed to load authentication status', 'Auth status failed:');
      }
    },

    // Mounted behind the auth middleware, so req.user is always present.
    async changePassword(req, res) {
      try {
        if (!req.user?.id) {
          return badRequest(res, 'Password changes require an account session');
        }
        const currentPassword = typeof req.body?.currentPassword === 'string' ? req.body.currentPassword : '';
        const newPassword = typeof req.body?.newPassword === 'string' ? req.body.newPassword : '';
        if (newPassword.length < 8) {
          return badRequest(res, 'New password must be at least 8 characters');
        }
        const verified = await accounts.verifyLogin(req.user.username, currentPassword);
        if (!verified) {
          return unauthorized(res, 'Current password is incorrect');
        }
        await accounts.changePassword(req.user.id, newPassword);
        // Revoke every other session so a stolen device loses access.
        await sessionStore.destroyAllForUser(req.user.id, { exceptToken: req.user.sessionToken });
        return res.status(204).end();
      } catch (err) {
        return storageError(res, err, 'Failed to change password', 'Password change failed:');
      }
    },

    // Mounted behind the auth middleware. Lists the caller's own live sessions.
    async listSessions(req, res) {
      try {
        if (!req.user?.id || !sessionStore) return res.json({ sessions: [] });
        const sessions = await sessionStore.listForUser(req.user.id, { currentToken: req.user.sessionToken });
        return res.json({ sessions });
      } catch (err) {
        return storageError(res, err, 'Failed to list sessions', 'Session listing failed:');
      }
    },

    // Revokes one of the caller's other sessions. The current session is
    // ended through /api/auth/logout so the cookie is cleared as well.
    async revokeSession(req, res) {
      try {
        if (!req.user?.id || !sessionStore) {
          return badRequest(res, 'Session management requires an account session');
        }
        const sessionId = typeof req.body?.sessionId === 'string' ? req.body.sessionId : '';
        const currentId = req.user.sessionToken ? hashSessionToken(req.user.sessionToken).slice(0, 16) : null;
        if (!sessionId || sessionId === currentId) {
          return badRequest(res, 'Choose another device to sign out');
        }
        const removed = await sessionStore.destroyById(req.user.id, sessionId);
        if (!removed) return notFound(res, 'That device is already signed out');
        return res.status(204).end();
      } catch (err) {
        return storageError(res, err, 'Failed to sign out that device', 'Session revocation failed:');
      }
    },

    async revokeOtherSessions(req, res) {
      try {
        if (!req.user?.id || !sessionStore) {
          return badRequest(res, 'Session management requires an account session');
        }
        await sessionStore.destroyAllForUser(req.user.id, { exceptToken: req.user.sessionToken });
        return res.status(204).end();
      } catch (err) {
        return storageError(res, err, 'Failed to sign out other devices', 'Session revocation failed:');
      }
    }
  };
}

module.exports = {
  DEFAULT_SESSION_TTL_MS,
  MAX_SESSION_TTL_MS,
  SESSION_RENEW_MIN_AGE_MS,
  PUBLIC_API_PATHS,
  SESSION_COOKIE,
  createAuthMiddleware,
  createAuthRoutes,
  createSession,
  createSessionStore,
  hashSessionToken,
  isPublicRequest,
  canonicalRequestPath,
  parseCookies,
  requestBearerToken,
  requestToken,
  requireAdmin,
  resolveRequestUser,
  sessionCookieOptions,
  secureCookiePolicy,
  requestIsHttps,
  timingSafeEqualStrings,
  validateSessionsStore,
  verifySession
};
