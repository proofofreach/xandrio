...
function mergeDividerNotesIntoExcerpt(chapters = [], options = {}) {
  ...
  for (let index = 0; index < chapters.length; index++) {
    ...
    if (
      !label ||
      !isPlaceholderSectionTitle(recipient?.title) ||
      String(donor?.text || '').length + String(recipient?.text || '').length + 2 > oversizedChars
    ) {
      merged.push(donor);
      continue;
    }
...
// Display metadata only: rawTitle keeps the original, so the chapter structure
// key — and every position keyed to it — is untouched.
function nameSectionsFromLeadingDividers(chapters = [], options = {}) {
  if (!Array.isArray(chapters)) return chapters;
  const thresholdChars = Number(options.thresholdChars) || DEGENERATE_SECTION_CHARS;
  return chapters.map((chapter, index) => {
    if (!index || !isPlaceholderSectionTitle(chapter?.title)) return chapter;
    const label = leadingDividerHeading(chapters[index - 1], thresholdChars);
...
function normalizeChapterSequence(chapters = [], options = {}) {
  ...
  const mergedSections = mergesDegenerateSections(options.sourceFormat)
    ? nameSectionsFromLeadingDividers(
      mergeDividerNotesIntoExcerpt(mergeDegenerateSections(normalizedMetadata))
    )
    : normalizedMetadata;
