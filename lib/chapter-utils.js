...
function deriveHeadingTitleFromText(text = '') {
  ...
  if (
    ...
    second &&
    second.length <= 180 &&
    !/^(?:Hamilton|Jay|Madison)(?:,\s+with\s+\w+)?:/i.test(second)
  ) {
...
function structuralChapterMetadata(chapter = {}, derivedTitle = '') {
  ...
  if (
    /^(?:table\s+of\s+)?contents\b/i.test(firstLine) ||
    (!trustedTocTitle && numberedContentsLines >= 3)
  ) {
...
function sequentialHeadingCandidate(chapter = {}) {
  const firstLine = String(chapter.text || '')
    .split(/\n+/)
    .map(line => line.replace(/\s+/g, ' ').trim())
    .find(Boolean) || '';
  const marker = firstLine.match(/^([IVXLCDM]+)(?:\s+\d{1,4})?$/i);
  if (!marker) return null;
  const number = romanNumeralToNumber(marker[1]);
  if (!number) return null;
  return {
    number,
    title: deriveHeadingTitleFromText(chapter.text) || marker[1].toUpperCase()
  };
}
...
function normalizeChapterSequence(chapters = []) {
  ...
  const repairedSeries = repairSequentialChapterSeries(chapters);
...
function splitOversizedChapters(chapters = [], options = {}) {
  ...
  for (const chapter of chapters) {
    const text = String(chapter?.text || '').trim();
    const hasAuthoredTocBoundary = Boolean(chapter?.fromToc && String(chapter.title || '').trim());
    if (!chapter || text.length <= thresholdChars || hasAuthoredTocBoundary) {
...
function repairTextArtifacts(text) {
  return String(text || '')
    ...
    .replace(/^([IVXLCDM]+)[ \t]+\d{1,4}[ \t]*(?=\r?$)/gimu, '$1')
