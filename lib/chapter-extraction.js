...
async function extractChapters(epubPath) {
  ...
  try {
      ...
      for (let i = 0; i < flow.length; i++) {
        ...
        spineItems.push({ spineIndex: i, id: item.id, href, title: item.title || '', heading, text: text.trim(), linear: true });
      }
      ...
      for (let i = 0; i < rawChapters.length; i++) {
        ...
        if (ch.text.length < 500 && !ch.fromToc && !ch.authoredBoundary) {
      ...
      const numberedChapterTitle = title =>
        /^(?:chapter\s+)?(?:\d+|[ivxlcdm]+)(?:[.:\-–—]|\s+)/i.test(String(title || '').trim());
      ...
      const backMatterTitle = title =>
        /^(?:dedication|acknowledgments?|epilogue|afterword|references|bibliography|selected bibliography|index|glossary|appendix|scripture index|general index|source notes?|endnotes?|works cited|further reading|suggestions? for further reading|recommended reading|reading group guide|discussion questions?|permissions?|credits?)/i
          .test(String(title || '').trim());
      const identifiedChapters = rawChapters.map((ch, index) => {
        let chapterType = 'content';
        const t = ch.title;

        if (ch.authoredChapter) {
          chapterType = 'chapter';
