  ...
  try {
      ...
      const identifiedChapters = rawChapters.map((ch, index) => {
        ...
        if (ch.authoredChapter) {
          ...
          chapterType = 'author';
        } else if (/^(dedication|acknowledgments?|epilogue|afterword|references|bibliography|selected bibliography|index|glossary|appendix|scripture index|general index|source notes?|endnotes?|works cited|further reading|suggestions? for further reading|recommended reading|reading group guide|discussion questions?|permissions?|credits?)/i.test(t)) {
