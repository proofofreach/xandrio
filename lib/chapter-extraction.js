...
const { stripHTML, normalizeAllCapsTitle, isChapterLikeTitle } = require('./chapter-utils');
...
function hrefFileName(href = '') {
  return decodeEpubHref(String(href).split('#')[0]).split('/').pop();
}

function documentSemantics(html = '', guideType = '') {
  const bodyAttributes = String(html).match(/<body\b([^>]*)>/i)?.[1] || '';
  const classValue = bodyAttributes.match(/\bclass\s*=\s*["']([^"']+)["']/i)?.[1] || '';
  const epubType = bodyAttributes.match(/\bepub:type\s*=\s*["']([^"']+)["']/i)?.[1] || '';
  const tokens = new Set(`${classValue} ${epubType}`.toLocaleLowerCase('en').split(/\s+/).filter(Boolean));
  const normalizedGuideType = String(guideType).toLocaleLowerCase('en');
  const hasTocNavigation = /<(?:nav|section)\b[^>]*\bepub:type\s*=\s*["'][^"']*\btoc\b[^"']*["']/i
    .test(String(html));

  if (normalizedGuideType === 'copyright-page' || tokens.has('copyright')) {
    return { type: 'copyright', title: 'Copyright' };
  }
  if (normalizedGuideType === 'toc' || tokens.has('toc') || tokens.has('contents') || hasTocNavigation) {
    return { type: 'toc', title: 'Contents' };
  }
  if (tokens.has('dedication')) return { type: 'frontmatter', title: 'Dedication' };
  if (
    tokens.has('preface') ||
    tokens.has('foreword') ||
    tokens.has('introduction') ||
    tokens.has('frontmatter') ||
    tokens.has('otherbooks') ||
    tokens.has('other-books')
  ) {
    return { type: 'frontmatter' };
  }
  if (
    tokens.has('appendix') ||
    tokens.has('backmatter') ||
    tokens.has('bibliography') ||
    tokens.has('notes') ||
    tokens.has('index')
  ) {
    return { type: 'backmatter' };
  }

  const attributionCount = (String(html).match(/\bclass\s*=\s*["'][^"']*\battribution\b[^"']*["']/gi) || []).length;
  if (attributionCount >= 3) return { type: 'frontmatter' };
  return null;
}
...
const PRINTED_CHAPTER_NUMBERS = new Map([
  ['ONE', 1],
  ['TWO', 2],
  ['THREE', 3],
  ['FOUR', 4],
...
function printedChapterNumber(text = '') {
  const compact = String(text)
    .normalize('NFKC')
    .replace(/[^\p{L}\p{N}]+/gu, '')
    .toUpperCase();
  if (!compact) return null;
  if (/^\d{1,2}$/.test(compact)) return Number(compact);
  if (PRINTED_CHAPTER_NUMBERS.has(compact)) return PRINTED_CHAPTER_NUMBERS.get(compact);
  return romanNumeralToNumber(compact);
}

function recoveredHeadingAt(bolds, index) {
  const first = bolds[index];
  if (!first) return '';
  let title = first.text;
  const next = bolds[index + 1];
  if (!next || next.pos - first.end > 180) return title;

  const continuesTitle = /[:\-–—]$/.test(title) || /^[a-z]/.test(next.text);
  if (continuesTitle && `${title} ${next.text}`.length <= 140) {
    title = `${title} ${next.text}`;
  }
  return title;
}

function printedChapterHeadingSequence(html, bolds, expectedCount) {
  ...
  for (const paragraph of paragraphs) {
    const number = printedChapterNumber(paragraph.text);
    if (!number || number > expectedCount) continue;
    while (boldIndex < bolds.length && bolds[boldIndex].pos <= paragraph.end) boldIndex++;
    const bold = bolds[boldIndex];
    if (!bold || bold.pos - paragraph.end > 500) continue;
    if (stripHTML(html.slice(paragraph.end, bold.pos)).trim()) continue;
    candidates.push({
      number,
      pos: paragraph.pos,
      heading: recoveredHeadingAt(bolds, boldIndex)
    });
  }

  const candidatesByNumber = new Map();
  for (const candidate of candidates) {
    if (!candidatesByNumber.has(candidate.number)) candidatesByNumber.set(candidate.number, []);
    candidatesByNumber.get(candidate.number).push(candidate);
  }
  let best = null;
  let examined = 0;

  function visit(number, sequence) {
    if (examined >= 2000) return;
    if (number > expectedCount) {
      examined++;
      const scored = scoreCandidateSequence(sequence, expectedCount);
      if (scored && (!best || scored.score < best.score)) {
        best = { sequence: [...sequence], score: scored.score };
      }
      return;
    }
    const previousPos = sequence[sequence.length - 1]?.pos ?? -1;
    for (const candidate of candidatesByNumber.get(number) || []) {
      if (candidate.pos <= previousPos) continue;
      sequence.push(candidate);
      visit(number + 1, sequence);
      sequence.pop();
    }
  }
...
const RECOVERED_BACK_MATTER_TITLES = new Map([
  ['afterword', 'Afterword'],
  ['epilogue', 'Epilogue'],
...
function recoveredBackMatterSections(html, start, sourceItem) {
  const markers = [...html.matchAll(/<p\b[^>]*>([\s\S]*?)<\/p>/gi)]
    .map(match => {
      const text = stripHTML(match[1]).replace(/\s+/g, ' ').trim();
      return {
        pos: match.index,
        title: RECOVERED_BACK_MATTER_TITLES.get(text.toLocaleLowerCase('en'))
      };
    })
    .filter(marker => marker.pos > start && marker.title);
  const firstIndex = markers.findIndex((marker, index) => {
    const end = markers[index + 1]?.pos ?? html.length;
    return stripHTML(html.slice(marker.pos, end)).trim().length >= 200;
  });
  if (firstIndex < 0) return null;

  const relevant = markers.slice(firstIndex);
  const substantive = relevant.filter((marker, index) => {
    const end = relevant[index + 1]?.pos ?? html.length;
    return stripHTML(html.slice(marker.pos, end)).trim().length >= 200;
  });
  const sections = [];
  for (let index = 0; index < substantive.length; index++) {
    const marker = substantive[index];
    const end = substantive[index + 1]?.pos ?? html.length;
    const text = stripHTML(html.slice(marker.pos, end)).trim();
    sections.push({
      title: marker.title,
      text,
  ...
  return sections.length > 0
    ? { start: substantive[0].pos, sections }
    : null;
}
...
function recoverScannedChapterBundle({ sourceItem, html, tocEntries, tocChapters }) {
  ...
  let boundaries = printedChapterHeadingSequence(html, bolds, plan.expectedCount);
  if (!boundaries) {
    ...
    for (let imageIndex = 0; imageIndex < images.length; imageIndex++) {
      ...
      signatureGroups.get(signature).push({
        ...image,
        heading: recoveredHeadingAt(bolds, boldIndex)
      });
    }
  ...
  const backMatter = recoveredBackMatterSections(
    html,
    boundaries[boundaries.length - 1].pos,
    sourceItem
  );
  const chapters = [];
  for (let index = 0; index < boundaries.length; index++) {
    const start = boundaries[index].pos;
    const end = boundaries[index + 1]?.pos ?? backMatter?.start ?? html.length;
    const text = stripHTML(html.slice(start, end)).trim();
    if (text.length < 500) return null;
    const chapterNumber = index + 1;
    chapters.push({
      title: boundaries[index].heading ||
        plan.titleByNumber.get(chapterNumber) ||
        `Chapter ${chapterNumber}`,
      text,
      originalIndex: sourceItem.spineIndex,
      fromToc: true,
      authoredChapter: true,
      recoveredFromScannedBundle: true
    });
  }
  if (backMatter) chapters.push(...backMatter.sections);
...
// Scan navigation often names covers and page containers, not chapters. Only
// replace it when page-start number/title pairs establish an ordered sequence.
function hasChapterNavigation(toc = []) {
  return toc.some(entry => {
    const title = stripHTML(entry.title || '').trim();
    return title && !/^(?:(?:front |back )?cover|title page|copyright(?: page)?|pages?|(?:table of )?contents|start|dedication|acknowledg(?:e)?ments|preface|foreword|introduction)$/i.test(title);
  });
}

async function recoverPageScanChapters(epub, spineItems, rawChapters) {
  const previousText = collapseUnicodeText(rawChapters.map(ch => ch.text).join(' '));
  const documents = [];
  for (const item of spineItems) {
    if (!item.text || !previousText.includes(collapseUnicodeText(item.text))) continue;
    documents.push({ ...item, html: await getChapterHtml(epub, item.id) });
  }
  const html = documents.map(item => item.html).join('\n');
  const pages = [];
  const markers = [...html.matchAll(/<(?:div|span|a)\b[^<>]*\bid\s*=\s*["']page[-_]\d+["'][^<>]*>/gi)];
  for (let i = 0; i < markers.length; i++) {
    const marker = markers[i];
    const end = Math.min(markers[i + 1]?.index ?? html.length, marker.index + 8192);
    const paragraphs = [...html.slice(marker.index + marker[0].length, end)
      .matchAll(/<p\b[^>]*>([\s\S]*?)<\/p>/gi)].slice(0, 3)
      .map(match => stripHTML(match[1]).replace(/\s+/g, ' ').trim());
    if (!paragraphs[0]) continue;
    pages.push({ pos: marker.index, paragraphs });
  }
  const titleLike = text => text && text.length >= 3 && text.length <= 140 &&
    /\p{L}/u.test(text) && !/[.!?;,]$/.test(text) && !/^\d+$/.test(text);
  const sequence = [];
  for (const page of pages) {
    const [label, title, prose] = page.paragraphs;
    const number = printedChapterNumber(label);
    if (!number || number > 100 || !titleLike(title) || !prose || prose.length < 100) continue;
    const previous = sequence.at(-1);
    if (previous?.number === number && previous.heading === title) continue;
    sequence.push({ ...page, number, heading: title, title: `Chapter ${number}: ${title}` });
  }
  if (sequence.length < 3 || sequence[0].number > 2 ||
      new Set(sequence.map(entry => entry.heading)).size !== sequence.length ||
      sequence.some((entry, i) => i > 0 && entry.number !== sequence[i - 1].number + 1)) return null;

  // A scan can lose the first chapter numeral. Require its title to recur as
  // a running header after Part One, with a page number, before chapter two.
  if (sequence[0].number === 2) {
    const frontPages = pages.filter(page => page.pos < sequence[0].pos);
    const firstPartIndex = frontPages.findLastIndex(page => /^part\s+(?:1|i|one)$/i.test(page.paragraphs[0]));
    if (firstPartIndex < 0) return null;
    const preceding = frontPages.slice(firstPartIndex + 1);
    const first = preceding.find((page, index) => {
      const [title, prose] = page.paragraphs;
      return titleLike(title) && prose?.length >= 100 &&
        preceding.slice(index + 1).some(later => later.paragraphs[0] === title &&
          /^\d+$/.test(later.paragraphs[1] || ''));
    });
    if (!first) return null;
    sequence.unshift({ ...first, number: 1, title: `Chapter 1: ${first.paragraphs[0]}` });
  }

  const boundaries = [...sequence];
  const seenAuxiliary = new Set();
  for (const page of pages) {
    const [first, second] = page.paragraphs;
    let title;
    if (page.pos < sequence[0].pos && /^(?:introduction|preface|foreword):?$/i.test(first)) {
      title = first.replace(/:$/, '') + (titleLike(second) ? `: ${second}` : '');
    } else if (/^part\s+(?:\d+|[ivxlcdm]+|one|two|three|four|five)$/i.test(first) && titleLike(second)) {
      title = `${first}: ${second}`;
    } else if (page.pos > sequence.at(-1).pos &&
      (/^postscript\b/i.test(first) || RECOVERED_BACK_MATTER_TITLES.has(first.toLowerCase()))) {
      title = first;
    }
    if (title && !seenAuxiliary.has(title)) {
      seenAuxiliary.add(title);
      boundaries.push({ pos: page.pos, title, semanticType:
        RECOVERED_BACK_MATTER_TITLES.has(first.toLowerCase()) || /^postscript\b/i.test(first)
          ? 'backmatter' : undefined });
    }
  }
  // Preserve front matter as separate documents; never let "Contents" own
  // the body. Every split is checked against the prior narration below.
  const firstBoundary = Math.min(...boundaries.map(boundary => boundary.pos));
  let offset = 0;
  for (const document of documents) {
    document.start = offset;
    if (offset < firstBoundary) {
      const isContents = /contents/i.test(document.title) ||
        /^\s*(?:Contents\b|.*\s\d+\s*\n)/i.test(document.text);
      boundaries.push({ pos: offset, title: isContents ? 'Contents' : 'Front matter',
        semanticType: isContents ? 'toc' : 'frontmatter' });
    }
    offset += document.html.length + 1;
  }
  boundaries.sort((a, b) => a.pos - b.pos);
  const recovered = boundaries.map((boundary, index) => ({
    title: boundary.title,
    semanticType: boundary.semanticType,
    originalIndex: documents.findLast(document => document.start <= boundary.pos)?.spineIndex,
    text: stripHTML(html.slice(boundary.pos, boundaries[index + 1]?.pos ?? html.length)).trim(),
    recoveredFromPageScan: true,
    authoredBoundary: true,
    authoredChapter: Boolean(boundary.number),
    titleSource: 'printed-page-heading'
  })).filter(ch => ch.text);
  if (collapseUnicodeText(recovered.map(ch => ch.text).join(' ')) !== previousText) return null;
  return recovered;
}
...
async function extractChapters(epubPath) {
  ...
  try {
      ...
      const guideTypeByHref = new Map();
      for (const guideEntry of epub.guide || []) {
        const fileName = hrefFileName(guideEntry.href);
        if (fileName && guideEntry.type) guideTypeByHref.set(fileName, guideEntry.type);
      }
      ...
      for (let i = 0; i < flow.length; i++) {
        ...
        if (item.linear === false) {
          // EPUB itemrefs marked linear="no" are auxiliary documents such as
          // popup footnotes. They remain addressable from the reading content
          // but are not part of the default sequential reading order.
          spineItems.push({
            spineIndex: i,
            id: item.id,
            href,
        ...
        const text = stripHTML(html, { mutationRecorder: mutationCollector.record }).trim();
        ...
        const semantics = documentSemantics(html, guideTypeByHref.get(href));
        spineItems.push({
          spineIndex: i,
          id: item.id,
          href,
          ...
          text: text.trim(),
          linear: true,
          semanticBoundary: Boolean(semantics),
          semanticType: semantics?.type,
          semanticTitle: semantics?.title
        });
      }

      const substantialHeadingCount = spineItems.filter(item =>
        item.linear !== false && item.heading && item.text.length >= 500
      ).length;
      for (const item of spineItems) {
        // A single giant document may carry one book-title heading and still
        // need audiobook-sized fallback splits. Multiple substantial headed
        // spine documents, or an explicitly numbered heading, establish that
        // the publisher used the spine as authored section boundaries.
        item.authoredBoundary = Boolean(
          item.semanticBoundary ||
          (item.heading && (substantialHeadingCount >= 2 || isChapterLikeTitle(item.heading)))
        );
      }
      ...
      if (epub.toc && epub.toc.length > 0 && hasChapterNavigation(epub.toc)) {
        // Resolve each TOC entry to a spine index
        const tocEntries = [];
        // Explicit TOC levels distinguish sibling sections from ancestors.
        // Flat TOCs still need the Part/Book context heuristics below.
        const hasTocHierarchy = epub.toc.some(entry => Number.isInteger(entry.level) && entry.level > 0) &&
          epub.toc.every(entry => Number.isInteger(entry.level) && entry.level >= 0);
        let tocAncestors = [];
        for (const tocEntry of epub.toc) {
          ...
          let parentTitles;
          if (hasTocHierarchy) {
            tocAncestors = tocAncestors.filter(entry => entry.level < tocEntry.level);
            parentTitles = tocAncestors.map(entry => entry.title);
            tocAncestors.push({ level: tocEntry.level, title });
          }
          ...
          if (spineIdx !== undefined && spineItems[spineIdx]?.linear !== false) {
            tocEntries.push({ title, spineIdx, fragment, tocAncestors: parentTitles });
          }
        }
        ...
        if (duplicateTargetCount > 0) {
          mutationCollector.record({
            code: ALLOWED_TEXT_MUTATIONS.EXACT_DUPLICATE_REMOVAL.code,
        ...
        // Calibre can split one authored section into hundreds of physical
        // files. Count its logical documents as well as physical targets;
        // several anchors in one document must not inflate logical coverage.
        const logicalHref = item => decodeEpubHref(item.href).replace(/_split_\d+(?=\.[^.]+$)/i, '');
        const contentItems = spineItems.filter(s => s.text.length >= 50);
        const contentDocuments = new Set(contentItems.map(logicalHref));
        const referencedDocuments = new Set(dedupedToc
          .map(entry => logicalHref(spineItems[entry.spineIdx]))
          .filter(href => contentDocuments.has(href)));
        const tocCoverage = dedupedToc.length / Math.max(contentItems.length, 1);
        const logicalCoverage = referencedDocuments.size / Math.max(contentDocuments.size, 1);

        if (dedupedToc.length >= 3 && Math.max(tocCoverage, logicalCoverage) >= 0.3) {
          ...
          async function getSpineHtml(spineIdx) {
            ...
            if (spineItems[spineIdx]?.linear === false) {
              spineHtmlCache[spineIdx] = '';
              return '';
            }
          ...
          for (let ti = 0; ti < dedupedToc.length; ti++) {
            ...
            if (nextEntry && nextEntry.spineIdx === entry.spineIdx && (entry.fragment || nextEntry.fragment)) {
              ...
              try {
                ...
                for (let ei = 0; ei < sameSpineEntries.length; ei++) {
                  ...
                  const chEntry = {
                    title: e.title,
                    tocAncestors: e.tocAncestors,
                    text,
                    originalIndex: entry.spineIdx,
                    fromToc: true,
                    // An explicit anchor is an authored boundary even when its
                    // range is empty. Do not let the empty-title-page repair
                    // steal text from the following anchored section.
                    anchorBounded: true,
                ...
                rawTocChapters.push({
                  title: entry.title,
                  tocAncestors: entry.tocAncestors,
            ...
            // A TOC range is an authored boundary regardless of its byte
            // size or number of source files. TTS chunks long narration later.
          ...
          for (let i = 0; i < rawTocChapters.length - 1; i++) {
            ...
            if (curr.text.length === 0 && next.text.length > 500 && !curr.anchorBounded) {
          ...
          if (dedupedToc.length > 0 && dedupedToc[0].spineIdx > 0) {
            ...
            const preTocChapters = preTocTexts.flatMap(si => {
              ...
              return [{
                ...
                fromToc: false,
                semanticType: si.semanticType,
                semanticTitle: si.semanticTitle
              }];
            });
      ...
      if (tocChapters) {
        ...
        rawChapters = spineItems.map((si, i) => {
          ...
          let titleSource = title ? (si.title ? 'spine' : 'toc-href') : '';
          ...
          if (!title && si.semanticTitle) {
            title = si.semanticTitle;
          ...
          return {
            ...
            fromToc: false,
            titleSource,
            authoredBoundary: si.authoredBoundary,
            semanticType: si.semanticType
          };
        }).filter(ch => ch.text.length >= 50 || isTitleLikeText(ch.text) || ch.authoredBoundary);
      ...
      if (!hasChapterNavigation(epub.toc)) {
        rawChapters = await recoverPageScanChapters(epub, spineItems, rawChapters) || rawChapters;
      }

      // A TOC parent can point to only its heading before its first child.
      // Keep the heading in reading order without exposing an unplayable entry.
      for (let i = rawChapters.length - 2; i >= 0; i--) {
        const parent = rawChapters[i];
        const child = rawChapters[i + 1];
        const heading = collapseUnicodeText(parent.title).toLocaleLowerCase('en');
        if (parent.fromToc && heading &&
            collapseUnicodeText(parent.text).toLocaleLowerCase('en') === heading &&
            child.tocAncestors?.some(title => collapseUnicodeText(title).toLocaleLowerCase('en') === heading)) {
          child.text = `${parent.text}\n\n${child.text}`;
          rawChapters.splice(i, 1);
        }
      }
      ...
      for (let i = 0; i < rawChapters.length; i++) {
        ...
        const mergeableAuthoredDivider = /^(?:part|book|volume)\b/i.test(String(ch.title || '').trim());
        if (
          ch.text.length < 500 &&
          !ch.fromToc &&
          !ch.recoveredFromPageScan &&
          (!ch.authoredBoundary || mergeableAuthoredDivider)
        ) {
          ...
          if (pendingShort.length > 0) {
            ...
            const shortTexts = pendingShort
              .map(s => /^(?:part|book|volume)\b/i.test(String(s.title || '').trim()) ? s.title : s.text)
              .filter(t => t.length > 0);
      ...
      const numberedChapterTitle = title => isChapterLikeTitle(title);
      const lastNumberedChapterIndex = rawChapters.reduce(
        (last, chapter, index) => numberedChapterTitle(chapter.title) ? index : last,
        -1
      );
      const backMatterTitle = title =>
        /^(?:sources(?: and notes)?|biographical notes)$/i.test(String(title || '').trim()) ||
        /^(?:epilogue|afterword|references|bibliography|selected bibliography|index|glossary|appendix|scripture index|general index|source notes?|a note on sources|endnotes?|notes?|works cited|further reading|suggestions? for further reading|recommended reading|reading group guide|discussion questions?|permissions?|credits?|photographic credits?)\b/i
          .test(String(title || '').trim());
      const explicitTocContexts = new WeakSet();
      const identifiedChapters = rawChapters.map((ch, index) => {
        let chapterType = 'content';
        const t = ch.title;

        if (ch.semanticType) {
          chapterType = ch.semanticType;
        } else if (ch.tocAncestors?.some(backMatterTitle)) {
          chapterType = 'backmatter';
        } else if (ch.tocAncestors?.some(title => /^(preface|foreword|introduction)\b/i.test(title))) {
          chapterType = 'frontmatter';
        } else if (ch.authoredChapter) {
          chapterType = 'chapter';
        } else if (/^(cover|title page)$/i.test(t)) {
          chapterType = 'cover';
        } else if (/^(copyright|publisher|isbn)/i.test(t)) {
          chapterType = 'copyright';
        } else if (/^(table of contents|contents)$/i.test(t)) {
          chapterType = 'toc';
        } else if (/^(preface|foreword|introduction)\b/i.test(t)) {
          chapterType = 'frontmatter';
        } else if (/^(?:acclaim|praise)\s+for\b/i.test(t) || /^also\s+by\b/i.test(t)) {
          chapterType = 'frontmatter';
          ...
          chapterType = 'author';
        } else if (backMatterTitle(t)) {
          chapterType = 'backmatter';
        } else if (
          /^notes?$/i.test(t) &&
          index > lastNumberedChapterIndex &&
          rawChapters.slice(index + 1).every(chapter => backMatterTitle(chapter.title))
        ) {
          // Endnotes follow the final numbered chapter and are followed only
          // by other back matter. This remains stable when auxiliary spine
          // documents inflate the raw item count.
          chapterType = 'backmatter';
        } else if (numberedChapterTitle(t)) {
          chapterType = 'chapter';
        ...
        if (ch.tocAncestors) {
          result.tocHierarchy = true;
          result.parentContext = ch.tocAncestors;
          explicitTocContexts.add(result);
        } else if (ch.parentContext) result.parentContext = ch.parentContext;
        if (ch.titleSource) result.titleSource = ch.titleSource;
        if (ch.authoredBoundary) result.authoredBoundary = true;
        return result;
      });
      ...
      // and Book > Part (War and Peace) hierarchies. Structural Part/Book/Volume
      // context belongs on every numbered chapter in that section, not only on
      // titles that happen to be duplicated elsewhere in the book.
      ...
      for (const ch of identifiedChapters) {
        ...
        } else if (ch.type === 'divider' && !explicitTocContexts.has(ch)) {
          // Generic divider — treat as innermost context level
          const idx = contextStack.findIndex(c => c === t);
          if (idx < 0) contextStack.push(t);
        }

        const hasStructuralContext = contextStack.some(context => /^(?:part|book|volume)\s+/i.test(context));
        const genericNumberedTitle = /^(?:(?:chapter|ch\.?)\s+(?:\d{1,3}|[ivxlcdm]+)|(?:\d{1,3}|[ivxlcdm]+)[.)]?)$/i.test(t);
        const needsContext = titleCounts[ch.title] > 1 ||
          (genericNumberedTitle && ((ch.type === 'chapter' && hasStructuralContext) ||
            (explicitTocContexts.has(ch) && contextStack.length > 0)));
        if (needsContext && !isAnyHeader && ch.type !== 'divider') {
    ...
    return attachMutationActivations(identifiedChapters, mutationCollector.values());
  } catch (error) {
    throw error;
  }
}
...
async function extractResult(epubPath) {
  const { fromLegacyChapters } = require('./extraction-result');
  return fromLegacyChapters(await extractChapters(epubPath), { sourceFormat: 'epub' });
}

module.exports = { extractResult, extractChapters, decodeEpubHref, getChapterHtml, getChapterText };
