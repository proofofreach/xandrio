...
function createCodexBookGuideProvider({
  ...
  function run(args, { input = '', signal = null, timeout = timeoutMs, maxBytes = maxResponseBytes } = {}) {
  ...
  async function beginLogin() {
    ...
    const consume = chunk => {
      ...
      login.userCode = login.output.match(/\b[A-Z0-9]{4,}(?:-[A-Z0-9]{4,})+\b/i)?.[0]?.toUpperCase() || '';
  ...
  async function generate({ modelSnapshot, prompt, purpose = 'generation', signal = null }) {
    const expected = await inspect({ model: modelSnapshot?.name, signal });
