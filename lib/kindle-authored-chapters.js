...
function recoverKindleAuthoredChapters(chapters) {
  ...
  for (let start = 0; start < chapters.length;) {
    ...
    let offset = 0;
    const origins = group.map(chapter => {
      const origin = { chapter, start: offset, end: offset + chapter.text.length };
      offset = origin.end + 2;
      return origin;
    });
    ...
    const recovered = found.map((m, i) => {
      ...
      const owner = origins.findLast(origin => origin.start <= m.index).chapter;
      const relevant = origins.filter(origin => origin.end > (i === 0 ? 0 : m.index) && origin.start < (found[i + 1]?.index ?? text.length));
      return { ...owner, title: `Chapter ${m[1]}`, text: value, estimatedDuration: estimateDuration(value),
        type: 'chapter', authoredBoundary: true, fromToc: true, boundarySource: 'source-toc-markers-v1',
        recoveredSourceSpineIds: [...new Set(relevant.map(origin => origin.chapter.sourceSpineId).filter(id => id !== undefined))] };
    });
