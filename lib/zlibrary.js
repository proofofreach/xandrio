...
// Upper bound on the provider-supplied `/eapi/info/domains` list that search()
// falls back to; that list has no length limit from Z-Library's side.
const DISCOVERED_DOMAIN_LIMIT = 5;
// How long getStatus() may serve a cached result instead of a fresh upstream
// round-trip. Short enough that operator-facing status stays accurate, long
// enough to collapse UI polling and duplicate download-preflight calls.
const STATUS_CACHE_TTL_MS = 5000;

let authFileMutex = Promise.resolve();

function serializeAuthFileOperation(operation) {
  const result = authFileMutex.then(operation, operation);
  authFileMutex = result.catch(() => {});
  return result;
}
...
class ZLibraryError extends Error {
  constructor(code, options = {}) {
    ...
    // Distinct from `retryable`: the same host must not be hammered again, but a
    // *different* mirror is exactly the right next move.
    if (options.mirrorBlocked !== undefined) this.mirrorBlocked = options.mirrorBlocked === true;
...
function createZLibraryClient(options = {}) {
  ...
  // A caller that passes fallbackBaseUrls has explicitly named the hosts it
  // trusts. The built-in list is only a convenience default for anonymous
  // search, so it is kept separate from the credentialed trust decision.
  const fallbackBaseUrlsExplicitlyConfigured = options.fallbackBaseUrls !== undefined;
  const configuredFallbackBaseUrls = !fallbackBaseUrlsExplicitlyConfigured
    ? BUILT_IN_BASE_URLS
  ...
  let authEpoch = 0;
  const nowMs = () => { const value = now(); return value instanceof Date ? value.getTime() : new Date(value).getTime(); };
  ...
  async function loadSession(epoch = authEpoch) {
    ...
    if (stored.version !== 2 || stored.email || stored.password || stored.loginAt || stored.baseUrl !== normalized.baseUrl) {
      const persisted = await persistSession(normalized, epoch);
      return persisted || null;
    }
    if (!authIsCurrent(epoch)) return null;
    session = normalized;
    anonymousBaseUrl = normalized.baseUrl;
    return session;
  }

  async function persistSession(value, epoch = authEpoch) {
    if (!authIsCurrent(epoch)) return null;
    ...
    return serializeAuthFileOperation(async () => {
      if (!authIsCurrent(epoch)) return null;
  ...
  async function connect({ email, password } = {}) {
    ...
    try {
      ...
      if (!authIsCurrent(epoch)) return disconnectedStatus();
    ...
    let profile;
    try {
      profile = await fetchProfile(candidate); // validate before persistence
    } catch (error) {
      if (!authIsCurrent(epoch)) return disconnectedStatus();
      throw error;
    }
    if (!authIsCurrent(epoch)) return disconnectedStatus();
    const persisted = await persistSession(candidate, epoch);
    if (!persisted || !authIsCurrent(epoch)) return disconnectedStatus();
  ...
  async function getStatus() {
    const epoch = authEpoch;
    const auth = await loadSession(epoch);
    if (!authIsCurrent(epoch) || !auth) return disconnectedStatus();
    if (cachedStatus && nowMs() - cachedStatusAt < STATUS_CACHE_TTL_MS) return cachedStatus;
    try {
      const validated = await fetchProfileWithRecovery(auth, epoch);
      if (!authIsCurrent(epoch) || !validated) return disconnectedStatus();
      ...
      if (!authIsCurrent(epoch)) return disconnectedStatus();
      if (error.code === 'ZLIB_AUTH_EXPIRED') {
        cachedStatus = null;
        cachedStatusAt = 0;
        return failedStatus('auth-expired', error, true);
      }
  ...
  async function search(query, opts = {}) {
    ...
    try {
      ...
      if (!shouldTryAnotherMirror(originalError)) throw originalError;
      const attempted = new Set([preferredBaseUrl]);
      // A domain that answers 200 with an empty book list has proved nothing:
      // Z-Library's bot wall serves exactly that to a blocked client. Pinning on
      // it strands every later search on a domain that can only return zero
      // results, for the lifetime of the process. Only a domain that actually
      // produced books is worth remembering; an empty answer is still returned
      // to the caller, it just does not get promoted.
      const tryBaseUrl = async baseUrl => {
        const books = await searchAtBaseUrl(baseUrl, params);
        if (books.length > 0) anonymousBaseUrl = baseUrl;
        return books;
      };
      for (const baseUrl of trustedFallbackBaseUrls(preferredBaseUrl)) {
        ...
        try {
          return await tryBaseUrl(baseUrl);
  ...
  async function download(result, destinationPath) {
    const epoch = authEpoch;
    const auth = await loadSession(epoch);
    if (!authIsCurrent(epoch) || !auth) throw new ZLibraryError('ZLIB_NOT_CONFIGURED');
    if (!result || !isSafeZlibId(result.zlibId) || !result.hash || !destinationPath) throw new ZLibraryError('ZLIB_PROTOCOL');
    const validated = await fetchProfileWithRecovery(auth, epoch);
    if (!authIsCurrent(epoch) || !validated) throw new ZLibraryError('ZLIB_NOT_CONFIGURED');
    const profile = validated.profile;
    ...
    if (!authIsCurrent(epoch)) throw new ZLibraryError('ZLIB_NOT_CONFIGURED');
  ...
  async function fetchProfileWithRecovery(auth, epoch = authEpoch) {
    try {
      const profile = await fetchProfile(auth);
      return authIsCurrent(epoch) ? { profile, auth } : null;
    } catch (originalError) {
      if (!authIsCurrent(epoch)) return null;
      if (!retryable(originalError)) throw originalError;
      // Only ever try ONE alternative. The session token is credential-bearing,
      // so a single transient failure must not spray it across every trusted
      // host in turn -- that is what let a hostile mirror harvest it by simply
      // outlasting the walk (see zlibrary-session-token-sprayed-across-all-
      // mirror-domains.md). Failing over once is still useful for an operator
      // who has configured a real backup; anything past that should surface
      // the original error instead of continuing to try hosts.
      const candidateBaseUrl = trustedAuthenticatedBaseUrls().find(baseUrl => baseUrl !== auth.baseUrl);
      if (!candidateBaseUrl) throw originalError;
      const candidate = { ...auth, baseUrl: candidateBaseUrl };
      try {
        const profile = await fetchProfile(candidate);
        if (!authIsCurrent(epoch)) return null;
        const persisted = await persistSession(candidate, epoch);
        return persisted ? { profile, auth: candidate } : null;
      } catch (error) {
        if (!authIsCurrent(epoch)) return null;
  ...
  function trustedAuthenticatedBaseUrls() {
    // Unlike the anonymous search fallback, credentialed requests must never be
    // sprayed across the hard-coded mirror list: those hostnames lapse and get
    // re-registered, and whoever holds one next would receive the session
    // token. Only hosts the operator named explicitly are trusted with it.
    const candidates = [configuredBaseUrl];
    if (fallbackBaseUrlsExplicitlyConfigured) candidates.push(...configuredFallbackBaseUrls);
    const urls = [];
    const seen = new Set();
    for (const value of candidates) {
      try {
        ...
        seen.add(baseUrl);
        urls.push(baseUrl);
      } catch { /* Ignore unsafe configured entries. */ }
    }
    return urls;
  }
  ...
  async function discoverDomains(preferredBaseUrl) {
    ...
    for (const seed of seeds) {
      try {
        ...
        // The provider chooses the length of this list; cap it so a hostile or
        // compromised mirror cannot pin the fallback loop in `search()` on an
        // unbounded, sequentially-probed candidate set.
        if (candidates.length) return candidates.slice(0, DISCOVERED_DOMAIN_LIMIT);
  ...
  async function jsonRequest(endpoint, request = {}) {
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
      try {
        const remote = await requestThroughWall(url, auth, {
          method,
        ...
        try {
          ...
          // An HTML body where JSON was promised is the edge's wall page, not the
          // API. Retrying this host is pointless, but the other mirrors are not
          // necessarily walled -- and treating this as terminal is what stopped
          // the client from ever reaching a mirror that was still serving.
          if (looksHtml(response, text)) {
            throw new ZLibraryError('ZLIB_UNAVAILABLE',
              { statusCode: response.status, retryable: false, mirrorBlocked: true });
          }
          let body;
  ...
  async function openDownloadTempFile(destinationPath) {
    for (;;) {
      const partPath = `${destinationPath}.${process.pid}.${Math.random().toString(16).slice(2)}.part`;
      try {
        return { partPath, handle: await fs.open(partPath, 'wx') };
      } catch (error) {
  ...
  async function streamDownload(initialUrl, destinationPath, auth) {
    ...
    try {
      ...
      const { partPath, handle } = await openDownloadTempFile(destinationPath);
      let bytes = 0;
      ...
      try {
        await pipeline(Readable.fromWeb(response.body), guard, handle.createWriteStream());
  ...
  // Z-Library's edge (DiamWall) gates the API with an ordinary cookie handshake:
  // the first request is answered 307 back to the same URL carrying a Set-Cookie,
  // and the retry that returns that cookie is served normally. This client sent
  // no cookie back and followed no hop, so every request died on the 307 and the
  // provider looked permanently unavailable -- on every domain at once, which is
  // what made this read as "search is broken" rather than "one mirror is down".
  //
  // Cookies are held per origin and never travel to another origin, so this adds
  // no new credential exposure: the redirect is only followed when it points at
  // the origin that issued it, and a cross-origin Location is refused outright.
  const wallCookies = new Map();

  function wallCookieHeader(origin) {
    const jar = wallCookies.get(origin);
    return jar && jar.size ? [...jar].map(([name, value]) => `${name}=${value}`).join('; ') : '';
  }

  function rememberWallCookies(origin, response) {
    const entries = response.headers?.getSetCookie?.() ?? [];
    const jar = wallCookies.get(origin) || new Map();
    let learned = false;
    for (const entry of entries) {
      const match = String(entry).match(/^\s*([^=;\s]+)=([^;]*)/);
      if (!match) continue;
      // remix_* are the session credentials, owned by loadSession/parseLogin.
      // The wall jar only ever holds the edge's own gate cookies.
      if (match[1].startsWith('remix_')) continue;
      if (jar.get(match[1]) !== match[2]) learned = true;
      jar.set(match[1], match[2]);
    }
    if (jar.size) wallCookies.set(origin, jar);
    return learned;
  }

  function headersWithWallCookies(auth, origin) {
    const headers = headersFor(auth);
    const wall = wallCookieHeader(origin);
    if (!wall) return headers;
    headers.Cookie = headers.Cookie ? `${headers.Cookie} ${wall}` : wall;
    return headers;
  }

  // One request, plus at most one replay to satisfy a same-origin cookie gate.
  async function requestThroughWall(url, auth, requestOptions, timeoutMs, rejectionCode) {
    const origin = new URL(url).origin;
    let remote = await secureRequest(url,
      { ...requestOptions, redirectAsResponse: true, headers: headersWithWallCookies(auth, origin) },
      timeoutMs, rejectionCode);
    const status = remote.response.status;
    if (status < 300 || status >= 400) return remote;
    const location = remote.response.headers?.get?.('location');
    const learned = rememberWallCookies(origin, remote.response);
    let target;
    try { target = location ? new URL(location, url) : null; } catch { target = null; }
    // Only a same-origin hop that actually handed us a new cookie is worth
    // replaying; anything else is a real redirect this client does not follow.
    // Refusing it must stay indistinguishable from the old behaviour, so a
    // credential-bearing request still fails closed on an off-origin Location.
    if (!learned || !target || target.origin !== origin) {
      remote.close();
      throw new ZLibraryError(rejectionCode || 'ZLIB_UNAVAILABLE',
        { statusCode: status, retryable: true });
    }
    remote.close();
    return secureRequest(target.toString(),
      { ...requestOptions, headers: headersWithWallCookies(auth, origin) }, timeoutMs, rejectionCode);
  }
  ...
  async function secureRequest(url, requestOptions, timeoutMs, rejectionCode) {
    try {
      return await requestRemote(url, {
        ...
        proxyUrl: options.proxyUrl || process.env.BOOK_PROXY_URL || undefined,
        timeoutMs,
        maxRedirects: requestOptions.maxRedirects ?? 0,
        // The API path follows nothing on its own; requestThroughWall inspects a
        // redirect and decides whether it is the edge's cookie gate.
        redirectAsResponse: requestOptions.redirectAsResponse === true,
...
// Same anchored shape as the local book-id guard elsewhere in the codebase
// (lib/request-guards.js `isSafeBookId`), applied here so `zlibId` cannot
// carry a dot-segment ("..") or separator into the request path this module
// builds by string interpolation.
const SAFE_ZLIB_ID_PATTERN = /^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/;
function isSafeZlibId(value) { return typeof value === 'string' && SAFE_ZLIB_ID_PATTERN.test(value); }
...
// Rotating to another mirror is worthwhile both for transient faults and for a
// mirror that answered with a wall page instead of the API.
function shouldTryAnotherMirror(error) { return retryable(error) || error?.mirrorBlocked === true; }
