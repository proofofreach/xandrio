...
function createZLibraryClient(options = {}) {
  ...
  async function getStatus() {
    ...
    try {
      const validated = await fetchProfileForAuth(auth, epoch);
  ...
  async function fetchProfileForAuth(auth, epoch = authEpoch) {
    // A session token is scoped to the origin that issued it. Automatic
    // credential failover turns transient availability into cross-origin
    // disclosure, even when every alternate is on a static trusted list.
    // Anonymous search may still discover and rotate public mirrors.
