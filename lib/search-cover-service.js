...
// v5 makes cached candidates pass the same visual-information gate as library
// covers, so blank scans from an older resolver cannot remain sticky.
const COVER_CACHE_SCHEMA = 'v5';
...
function createSearchCoverService(options = {}) {
  ...
  const inspectVisualQuality = options.inspectVisualQuality;
  ...
  function cachePath(key) {
    ...
    return path.join(cacheDir, path.basename(`${normalizedKey}.img`));
  }

  async function validCachedCover(key, { inspectVisual = false } = {}) {
    try {
      ...
      if (inspectVisual && inspectVisualQuality) {
        const visualQuality = await inspectVisualQuality(filePath);
  ...
  async function tryWriter(key, writer) {
    ...
    try {
      ...
      return validCachedCover(key, { inspectVisual: true });
    } catch {
