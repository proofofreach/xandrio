...
// `ownerKey` includes a client-chosen `owner` component (playback-routes.js),
// so a caller can defeat the single-session-per-owner replacement rule simply
// by varying it. This second cap is keyed on `rateKey` (the authenticated
// account or IP), which the caller does not control, so it holds even when
// `ownerKey` is being cycled specifically to dodge eviction.
const DEFAULT_MAX_ACTIVE_SESSIONS_PER_ACCOUNT = 4;
...
function createHlsAudioStreamer({
  ...
  maxActiveSessionsPerAccount = positiveInteger(
    process.env.XANDRIO_HLS_MAX_ACTIVE_SESSIONS_PER_ACCOUNT,
    DEFAULT_MAX_ACTIVE_SESSIONS_PER_ACCOUNT
  ),
  ...
  function createSession(key, ownerKey, rateKey, createSource) {
    ...
    // Enforce the per-account cap regardless of what `ownerKey` claims: evict
    // this account's own oldest running sessions first, rather than trusting a
    // client-chosen `owner` value to be honest about identity.
    if (maxActiveSessionsPerAccount > 0) {
      const accountRunning = [...sessionsById.values()].filter(session => (
        session.running
        && !session.evicted
        && session.id !== prior?.id
        && session.accountKey === accountKey
      ));
      if (accountRunning.length >= maxActiveSessionsPerAccount) {
        const oldestFirst = accountRunning.sort((left, right) => left.lastAccessAt - right.lastAccessAt);
        const overflow = accountRunning.length - maxActiveSessionsPerAccount + 1;
        for (const session of oldestFirst.slice(0, overflow)) void evictSession(session);
      }
    }
  ...
  async function serveSegment(req, res, sessionId, fileName) {
    ...
    // Two different failures used to share one status code, and the difference
    // matters to the player. A segment that is merely not written yet is
    // transient — the encoder is still running and the next poll will find it.
    // A session that no longer exists is permanent: its directory was removed,
    // so every remaining segment of that stream is gone too.
    // Reporting the second as 404 let a player that resumed after a long pause
    // retry a stream that could never come back, which is how an iOS listener
    // ended up in silence with the client still polling (see the stall
    // watchdog in public/js/single-file-chapter-player.js). 410 says "stop
    // retrying this stream and rebuild it" and shows up in the access log as
    // exactly that.
    if (!session) return res.status(410).json({ error: 'HLS playback session expired' });
    touch(session);
